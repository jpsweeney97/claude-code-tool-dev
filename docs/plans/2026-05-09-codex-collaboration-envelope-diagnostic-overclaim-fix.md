...
- A sibling-JSON in-place interpretive correction: mutate the three classification fields to rebaseline-vocabulary values. No structural schema changes (`_legacy_*` renaming, vocabulary markers, or dual-shape blocks). The JSON is a repo-internal artifact with zero confirmed production consumers; git history preserves the prior vocabulary.
...
**JSON disposition rationale (simplified from preserve-and-add).** The diagnostic JSON is a repo-internal artifact with zero confirmed production consumers (Task 1.4 verifies this). Git history preserves the prior vocabulary. In-place correction of interpretive fields is proportional to the risk: no reader will silently break because no reader exists. The vocabulary succession is documented in the commit message and the Markdown diagnostic's corrected wording.
...
Expected: zero production consumers (code under `packages/`, `scripts/`, `.claude/hooks/`, `extensions/` that reads this JSON file by path). Peer diagnostic JSONs containing the same field names as their own data are NOT consumers. If a production consumer IS found → fire the "JSON correction is unsafe" stop condition. Otherwise, record "no documented machine consumer at this revision" and proceed.
...
Mutate the three classification fields in place. No structural schema changes — no `_legacy_*` renaming, no `classification_vocabulary` markers, no dual-shape blocks. Git history preserves the prior vocabulary.
...
After editing, the body should contain only the sections that match the actually-staged file set from Steps 2.8, 3.7 (conditional), 4.6 (conditional), and 5.5 (state-based conditional, independent of Task 3). The opening parser-correction paragraph and closing scope-unchanged paragraph are always retained.
