...
| # | Lock | Rationale |
|---|---|---|
| L1 | `poll()` catches ONLY `UnknownKindInEscalationProjection`. No generic `except Exception`. | Narrow catch per spec §347: "each callsite owns its own abort-signaling and rejection semantics." Generic catch would mask other bugs. |
...
| L3 | On catch: (1) `logger.critical(...)` with `job_id`, `request_id`, `cause`; (2) `self._registry.signal_internal_abort(refreshed.parked_request_id, reason="unknown_kind_in_escalation_projection")`; (3) set `pending_escalation = None`. | Spec §439: "poll() returns DelegationPollResult(pending_escalation=None, ...) regardless of the signal's return." Matches `start()` precedent at `:797-808` for log structure. |
| L4 | `signal_internal_abort` return value is observational only. Do NOT branch the response shape on it. | Spec §436: CAS race with operator's `decide()` is honest concurrency; poll response is fixed. |
...
| L8 | No modifications to `_project_pending_escalation`, `_project_request_to_view`, or any other helper. Callsite-only change. | Helpers stay pure per spec §Internal abort coordination `:354`: "the helper never calls signal_internal_abort and never logs critical errors; those concerns live at callsites." |
