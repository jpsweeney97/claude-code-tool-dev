...
3. **Staleness** — Documentation updates aren't immediately available during a session

## Solution

A TypeScript MCP server that serves extension-reference documentation as searchable chunks. Claude Code connects via stdio and uses two tools:

1. **`search_extension_docs`** — Retrieve relevant documentation on demand via BM25 search
2. **`reload_extension_docs`** — Force reload after editing docs to refresh the search index
...
│  └─────────────────────────┬───────────────────────────┘│
│  ┌─────────────────────────▼───────────────────────────┐│
│  │  search_extension_docs()  │  reload_extension_docs()││
│  └─────────────────────────────────────────────────────┘│
└─────────────────────────────────────────────────────────┘
```
...
## Audit Gap Fixes (2026-01-12)

| ID  | Finding                                              | Severity | Resolution                                                            |
| --- | ---------------------------------------------------- | -------- | --------------------------------------------------------------------- |
| G1  | Zod schemas as plain objects may not compile         | High     | Wrap schemas in `z.object()` for SDK compatibility                    |
| G2  | Staleness not solved (only retry-on-failure)         | Major    | Add `reload_extension_docs` tool for manual refresh                   |
| G3  | `category`/`topic` not type-validated like `tags`    | Medium   | Add `typeof === 'string'` checks with warnings                        |
| G4  | Line count ≤150 is weak proxy for chunk size         | Low      | Add secondary guard on character count (≤8000 chars)                  |
...
**Decision:** Files ≤150 lines AND ≤8000 characters stay whole. Larger files go through a two-phase pipeline:

1. Split at H2 headings (outside code fences)
2. Merge small consecutive chunks (≤150 lines AND ≤8000 chars combined)

**Audit note (G4):** Line count alone is a weak proxy for chunk size — long lines (schemas, code blocks) can produce oversized chunks. The 8000-character limit (~2000 tokens at 4 chars/token) ensures chunks fit comfortably in retrieval contexts.
...
### Documentation Freshness: Manual Reload Tool

**Decision:** Add a `reload_extension_docs` tool for manual refresh during long sessions.

**Audit note (G2):** The original design claimed to solve "staleness" but only implemented retry-on-failure. If docs change during a session, search results remain stale until restart. A manual reload tool is the simplest solution that gives users explicit control.

**Why not file watching:** Cross-platform file watching is complex (debouncing, symlinks, network mounts). Manual reload is reliable and explicit.

**Why not periodic reload:** Implicit reloads during search could cause unexpected latency. Manual reload is predictable.

**Concurrent operations:** The reload tool reuses the existing `loadingPromise` mutex pattern. If a reload is called while a search-triggered load is in progress, it waits for that load to complete rather than starting a second concurrent load. If a search is called while a reload is in progress, it waits for the reload. This prevents race conditions and duplicate work.
...
function parseFrontmatter(
  ...
  try {
    ...
    // G3: Validate category and topic are strings (same rigor as tags)
...
function chunkFile(file: MarkdownFile): Chunk[] {
  ...
  // G4: Check both line count AND character count
...
// G4: Secondary size check — line count alone misses long lines (schemas, code blocks)
const MAX_CHUNK_CHARS = 8000; // ~2000 tokens at 4 chars/token

function isSmallEnoughForWholeFile(content: string): boolean {
  return countLines(content) <= 150 && content.length <= MAX_CHUNK_CHARS;
}
...
// G4: Check both line count AND character count when merging
function mergeSmallChunks(chunks: Chunk[], maxLines = 150, maxChars = MAX_CHUNK_CHARS): Chunk[] {
  const result: Chunk[] = [];
  let buffer: Chunk[] = [];
  let bufferLines = 0;
  let bufferChars = 0;
  ...
  for (const chunk of chunks) {
    ...
    if (bufferLines + lines <= maxLines && bufferChars + chars <= maxChars) {
      buffer.push(chunk);
      bufferLines += lines;
      bufferChars += chars;
    } else {
...
const EFFECTIVE_RETRY_INTERVAL =
  RETRY_INTERVAL_MS >= 1000 && RETRY_INTERVAL_MS <= 600000 ? RETRY_INTERVAL_MS : 60000; // U3: Default 60 seconds, configurable 1-600s
...
// G1: Use z.object() wrapper for SDK compatibility — plain shape objects may not work with all SDK versions.
// Ref: https://github.com/modelcontextprotocol/typescript-sdk — verify against installed version.
...
async function main() {
  ...
  // G1: Pass z.object() schemas — SDK extracts JSON Schema via zodToJsonSchema or similar.
  ...
  // G2: Reload tool for documentation freshness during long sessions
  server.registerTool(
    ...
    async () => {
      // Wait for any in-progress load to complete before clearing state
      // This prevents race conditions where we clear state mid-load
      if (loadingPromise) {
        console.error('Waiting for in-progress load to complete before reload...');
        await loadingPromise;
      }

      // Now safe to clear cached state and force fresh reload
      index = null;
      loadError = null;
      lastLoadAttempt = 0;
      ...
      if (!idx) {
        return {
          isError: true,
          content: [{ type: 'text', text: `Reload failed: ${loadError}` }],
        };
      }
        ...
        content: [
          {
            ...
            text: `Reloaded ${idx.chunks.length} chunks from documentation.${
              parseWarnings.length > 0
                ? ` Warning: ${parseWarnings.length} file(s) had parse issues.`
                : ''
            }`,
          },
        ],
...
```typescript
describe('reload_extension_docs', () => {
  it('clears cached index and reloads', async () => {
    // First search populates cache
    await client.callTool({ name: 'search_extension_docs', arguments: { query: 'hooks' } });

    // Modify docs (simulated by changing DOCS_PATH content)
    // ...

    // Reload
    const result = await client.callTool({ name: 'reload_extension_docs', arguments: {} });
    expect(result.isError).toBeFalsy();
    expect(result.content[0].text).toContain('Reloaded');
  });

  it('returns error if DOCS_PATH invalid after reload', async () => {
    process.env.DOCS_PATH = '/nonexistent';
    const result = await client.callTool({ name: 'reload_extension_docs', arguments: {} });
    expect(result.isError).toBe(true);
    expect(result.content[0].text).toContain('Reload failed');
  });

  it('waits for in-progress load before reloading', async () => {
    // Start a slow search that triggers initial load
    const searchPromise = client.callTool({
      name: 'search_extension_docs',
      arguments: { query: 'hooks' },
    });

    // Immediately call reload while search is loading
    const reloadPromise = client.callTool({
      name: 'reload_extension_docs',
      arguments: {},
    });

    // Both should complete successfully (reload waits for search's load)
    const [searchResult, reloadResult] = await Promise.all([searchPromise, reloadPromise]);
    expect(searchResult.isError).toBeFalsy();
    expect(reloadResult.isError).toBeFalsy();

    // Verify only 2 loads occurred (initial + reload), not 3
    // (Check stderr for "Loaded X chunks" messages)
  });
});
```

**Frontmatter type validation (G3):**

```typescript
describe('parseFrontmatter type validation', () => {
  // NOTE: parseWarnings is module-level state — clear before each test
  beforeEach(() => {
    parseWarnings.length = 0;
  });
  ...
  it('warns on non-string category', () => {
    ...
    expect(parseWarnings).toContainEqual({
      file: 'test.md',
  ...
  it('accepts valid string category and topic', () => {
    ...
    expect(parseWarnings).toHaveLength(0); // beforeEach clears, so 0 means no new warnings
  });
});
```

**Chunk size guards (G4):**

```typescript
describe('chunk size guards', () => {
  it('splits file with <=150 lines but >8000 chars', () => {
    // 60 lines total, but ~12,000 chars (exceeds 8000 limit)
    // Must include H2 headings for splitting to occur
    const longLine = 'x'.repeat(200);
    const section1 = ['## Section 1', ...Array(29).fill(longLine)].join('\n');
    const section2 = ['## Section 2', ...Array(29).fill(longLine)].join('\n');
    const content = `# Title\n\n${section1}\n\n${section2}`;
    const file = { path: 'test.md', content };
    const chunks = chunkFile(file);
    // Should split because total chars > 8000, even though lines < 150
    expect(chunks.length).toBeGreaterThan(1);
  });

  it('keeps file with <=150 lines and <=8000 chars as whole chunk', () => {
    const content = Array(50).fill('short line').join('\n');
    const file = { path: 'test.md', content };
    const chunks = chunkFile(file);
    expect(chunks).toHaveLength(1);
  });

  it('respects char limit when merging chunks', () => {
    ...
    const merged = mergeSmallChunks([chunk1, chunk2]);
    // Should NOT merge because 5000 + 5000 > 8000
...
21. **G1 verified:** Zod schemas use `z.object()` wrapper (not plain shape objects)
22. **G2 verified:** `reload_extension_docs` tool exists and clears index cache before reload
23. **G3 verified:** Non-string `category`/`topic` values produce warnings (not `[object Object]`)
24. **G4 verified:** Chunks respect both line count (≤150) AND character count (≤8000) limits
25. **Functional:** All 5 golden queries return expected top results
26. **Tests pass:** `npm test` runs successfully with vitest
