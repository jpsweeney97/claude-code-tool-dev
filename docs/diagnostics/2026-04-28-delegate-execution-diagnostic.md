...
| Field | Value |
|---|---|
| Branches fired | **S1 (Sandbox still blocked)** — primary signal from runtime-proof confirming `readableRoots: [worktree]` + `includePlatformDefaults: False` plus the parked `command_approval` for `/bin/zsh -lc` proving the sandbox blocked the shell command pre-execution. **S7 (Amendment required) — secondary, weak fire under loose interpretation only.** PendingRequestStore L1 (request 0 `op: create`) shows `requested_scope.proposedExecpolicyAmendment` populated with `["/bin/zsh", "-lc", "<chained-command>"]`. Per Symptom Attribution row S7's first clause ("Request payload contains `proposedExecpolicyAmendment`"), this triggers S7 by literal reading. **Caveat:** under `untrusted` approval policy, `proposedExecpolicyAmendment` appears in *every* `command_approval` request payload as Codex's standard offer of an amendment path — it is not a signal that progress is blocked on missing response shape. The S7 row's second clause ("requires `acceptWithExecpolicyAmendment`") is NOT met (App Server did not demand the amendment as the only valid response). Recorded for completeness; classification is unaffected (S1 still primary by precedence). The S7 row's "or" should be narrowed (see Hygiene next action #5). **Other branches did not fire:** no Packet 1 regression (control-plane state-machine + storage-evidence integrity verified via JSONL inspection); no calibration ratio signal (denominator < 3 per #1.d); no Approval-policy-noisy or Sandbox-patch-candidate triggers (smoke artifacts not produced). |
...
| Ticket/hygiene next action | (1) Update memory `feedback_deny_finalizes_job.md` with corrected mechanism (deny rejects ONE action; agent may iterate once before terminal). (2) Optional follow-up issue on the delegate's autopilot toward `.codex-collaboration/test-results.json`. (3) Audit prior session's handoff for the macOS `date -u -j -f` pitfall — same machine, same recipe template, likely recorded the same 4-hour-wrong UTC. (4) Open question on `available_decisions: []` in null-scope `file_change` — protocol shape quirk vs intentional restriction worth investigating before Candidate A so the next file_change escalation (if it surfaces) is interpretable. (5) **Narrow Symptom Attribution row S7's definition** — current "Request payload contains `proposedExecpolicyAmendment` *or* requires `acceptWithExecpolicyAmendment`" conflates two distinct cases: payload-presence (which under `untrusted` mode is universal across `command_approval` requests — Codex's standard amendment-offer mechanism) and response-required (genuinely amendment-blocked progress). Suggest split into S7a (payload-presence; informational) and S7b (response-required; classification-driving), or replace the "or" with stricter language so future variants don't auto-fire S7 on every `untrusted`-mode escalation. |
