...
| Missing context to apply a principle | Flag gap, proceed with best approximation, mark as incomplete |
...
- Overrides: Claude's default writing style when authoring instruction documents
- Scoped to: Instruction documents only (CLAUDE.md, skills, subagents)
- Does not override: Project-specific style guides for non-instruction content, user explicit instructions to deviate

**Precedence with other sources:**
- User explicit instruction > This document > Claude defaults
- If project CLAUDE.md has general style guidance that conflicts with these principles: these principles govern for instruction documents (task-specific beats general)
- If project CLAUDE.md explicitly defines alternative principles for instruction documents: project CLAUDE.md governs (project-specific beats task-specific)
...
| 9 | Define Authority | Declare precedence relationships with other instruction sources | Conflicting guidance without stated precedence |
| 10 | Close Loopholes | Anticipate and block creative misinterpretations | Rules without rationale, unaddressed edge cases |
| 11 | Front-Load | Put critical information first | Commands buried after context |
| 12 | Group Related | Keep conditions near consequences | Cross-references, scattered related content |
| 13 | Keep Parallel | Match structure across similar content | Mixed voice in lists, inconsistent hierarchy |
| 14 | Specify Outcomes | Define observable success criteria | "Ensure it works," process without verification |
| 15 | Economy | Remove words that don't advance meaning; use active voice | Filler phrases, passive voice, double negatives |
...
| 2 | State Boundaries (#5), Specify Failure Modes (#6), Specify Defaults (#7), Declare Preconditions (#8), Define Authority (#9) | Improvisation in ambiguous execution context is worse than verbosity |
| 3 | Close Loopholes (#10) | Misinterpretation harder to detect than fix |
...
Default behavior: Flag the gap and apply the document's stated default (see Specify Defaults). If no default is stated, stop and request clarification. Never silently improvise. Never spin indefinitely.
...
| "Read config from `config.yaml`" | "Requires: `config.yaml` last modified within 24 hours. Check: `stat -f %m config.yaml` > (now - 86400)." | Freshness |
...
| Grade | Criteria |
|-------|----------|
| A | All 15 principles followed consistently |
...
| Precedence ambiguity | #9 (Authority) | Override/defer/scope |
| Creative non-compliance | #10 (Loopholes) | Name and close rationalizations |
| Deprioritized information | #11 (Front-Load) | Lead with conclusions |
| Scattered reassembly | #12 (Group Related) | Physical = logical distance |
| Parsing overhead | #13 (Parallel) | Consistent structure |
| Unverifiable success | #14 (Outcomes) | Observable criteria |
| Signal dilution / context waste | #15 (Economy) | Remove non-advancing words |
