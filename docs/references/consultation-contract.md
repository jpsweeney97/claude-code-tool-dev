...
- OpenAI Codex via `mcp__codex__codex` and `mcp__codex__codex-reply` MCP tools
...
Run this pipeline immediately before every outbound Codex call (`mcp__codex__codex` or `mcp__codex__codex-reply`). This section is normative — skill and agent stubs must defer to it.
...
### New conversation (`mcp__codex__codex`)
