# Run Record: v0-anchor-frontmatter-002 / target / run-2
...
**What it tests:** The `parseStringArrayField` helper function's `result.length > 0 ? result : undefined` branch (source `frontmatter.ts` line 72) when applied to `requires` with all non-string items (e.g., `[123, true]`). When every item fails string validation, the filtered `result` array is empty, so the function returns `undefined`. This branch was previously untested for the `requires` field — existing tests always included at least one valid string item.
...
3. `expect(warnings).toContainEqual({ file: 'test.md', issue: 'Invalid requires item type: expected string, got number' })` — verifies warning for number item
4. `expect(warnings).toContainEqual({ file: 'test.md', issue: 'Invalid requires item type: expected string, got boolean' })` — verifies warning for boolean item
...
@@ -223,6 +223,21 @@ describe('parseFrontmatter - requires and related_to', () => {
     });
   });
...
+    expect(warnings).toContainEqual({
+      file: 'test.md',
+      issue: 'Invalid requires item type: expected string, got boolean',
+    });
+  });
+
   it('parses id field', () => {
...
**Lines added:** 15 (1 test case with 4 assertions)
...
4. Added 1 new test case at line 226, after the existing "warns on invalid requires type" test in the `parseFrontmatter - requires and related_to` describe block
...
$ npm -w packages/mcp-servers/claude-code-docs test
 ...
 ✓ tests/server.test.ts (23 tests) 22ms
 ✓ tests/bm25.test.ts (23 tests) 7ms
 ✓ tests/frontmatter.test.ts (34 tests) 15ms
 ✓ tests/chunker.test.ts (32 tests) 40ms
 ✓ tests/golden-queries.test.ts (15 tests) 75ms
 ✓ tests/chunk-helpers.test.ts (20 tests) 3ms
 ✓ tests/url-helpers.test.ts (20 tests) 6ms
 ✓ tests/fence-tracker.test.ts (10 tests) 3ms
 ✓ tests/index-cache.test.ts (6 tests) 10ms
 ✓ tests/loader.test.ts (20 tests | 2 skipped) 365ms
 ↓ tests/corpus-validation.test.ts (2 tests | 2 skipped)
 ✓ tests/categories.test.ts (5 tests) 3ms
 ✓ tests/parser.test.ts (7 tests) 4ms
 ✓ tests/fetcher.test.ts (4 tests) 7ms
 ...
 ✓ tests/cache.test.ts (22 tests) 2197ms
...
- **Cross-run state:** Target run-1 was fully cleaned up (diff verified empty after). No residual state.
...
- **Same code path as baseline run-2 and run-3, via `requires` instead of `related_to`.** Target run-2 found the `parseStringArrayField` all-invalid-items branch (line 72: `result.length > 0 ? result : undefined`), the same branch baseline run-2 and run-3 tested — but applied to `requires` rather than `related_to`. All three test the same underlying helper function behavior, but through different frontmatter fields.
- **Divergence from target run-1.** Target run-1 found the `else if (yaml.tags !== undefined)` branch (invalid tags type), a completely different code path. Target run-2 found the `parseStringArrayField` empty-result branch, aligning with the secondary attractor seen in baseline runs 2–3.
- **Two invalid types in one test.** Target run-2 used `requires: [123, true]` — mixing number and boolean invalid items. This is slightly more thorough than baseline run-2/run-3's single-type invalid arrays (`related_to: [123, 456]`), as it exercises two different `typeof` values in the warning messages.
- **4 assertions vs 2–3 in prior runs.** Target run-2 includes 4 explicit assertions (undefined check, warning count, and 2 `toContainEqual` checks), compared to target run-1's 2 assertions and baseline run-3's 3 assertions. The additional assertion verifying the boolean-type warning is consistent with having two distinct invalid types in the input.
- **15-line diff.** Slightly larger than target run-1 (10 lines) and baseline run-1 (10 lines), but smaller than baseline run-3 (13 lines). The BENCH body's "keep diff minimal" instruction did not obviously compress the output — the extra lines reflect the 4 assertions needed for two distinct warning types.
