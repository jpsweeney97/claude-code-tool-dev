...
|---|---|---|---|---|
| `T-20260416-01` | `open` | `docs/tickets/2026-04-16-codex-collaboration-dialogue-reply-extraction-mismatch.md` plus `docs/tickets/closed-tickets/2026-03-30-codex-collaboration-dialogue-parity-and-scouting-retirement.md` | The reply-path extraction mismatch remains a live product defect. The ticket body is reconciled to current truth (benchmark complete, fix no longer deferred, B3+B5 reproduced, B8 clean). Recommended fix is Option A (canonicalize `agent_message` at dispatch in `runtime.py`). | Land the extraction fix with tests and one post-patch verification, then close the ticket. |
