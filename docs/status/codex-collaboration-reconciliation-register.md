...
|---|---|---|---|---|
| `T-20260416-01` | `open` | `docs/tickets/2026-04-16-codex-collaboration-dialogue-reply-extraction-mismatch.md` plus `docs/tickets/closed-tickets/2026-03-30-codex-collaboration-dialogue-parity-and-scouting-retirement.md` | The reply-path extraction mismatch remains a live product defect. Ticket body revised with protocol analysis against vendored schema: the `turn/completed.turn.items[]` fallback was withdrawn (`Turn.items` is always empty per `TurnCompletedNotification.json:1285`). Recommended fix is now dispatch-layer canonicalization via `thread/read` fallback (mechanism A in ticket), with `item/completed` failure-mode investigation (mechanism C) as prerequisite. | Land the extraction fix with tests (including runtime integration test #4 and downstream regression test #5) and one post-patch verification, then close the ticket. |
