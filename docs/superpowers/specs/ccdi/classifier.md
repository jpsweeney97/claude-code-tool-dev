...
| Mid-dialogue | 1 high-confidence topic not yet in `injected` state (i.e., `detected` or `deferred` — absent-from-registry topics also qualify on first appearance), OR 1 medium-confidence leaf in 2+ consecutive turns | `injection.mid_turn_consecutive_medium_turns` (evaluated by the registry scheduling layer — see [registry.md#scheduling-rules](registry.md#scheduling-rules)) |
