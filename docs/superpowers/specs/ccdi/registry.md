...
| `deferred → deferred` (TTL=0, topic absent from classifier) | `deferred_ttl` ← `injection.deferred_ttl_turns` from config (reset). `last_seen_turn` is NOT updated — the topic was absent from classifier output this turn. `state`, `deferred_reason` also unchanged — only `deferred_ttl` is modified. |
...
|---------------------|-----------------|
| `weak_results` | `docs_epoch` differs from `suppressed_docs_epoch` (index updated since suppression — uses null comparison rules below) OR a new query facet is requested for the topic OR any semantic hint (`extends_topic`, `prescriptive`, or `contradicts_prior`) resolves to the suppressed topic (see [Semantic Hints](#semantic-hints)) |
