...
|-----------|----------------|
| `absent → detected` | `first_seen_turn` ← current turn, `last_seen_turn` ← current turn, `consecutive_medium_count` ← (1 if medium AND leaf-kind, else 0) (family-kind topics always initialize `consecutive_medium_count` ← 0 regardless of confidence), `coverage_target` ← from `ClassifierResult.resolved_topics[].coverage_target`, `facet` ← from `ClassifierResult.resolved_topics[].facet`. See [data-model.md defaults table](data-model.md#schema-defaults) for the schema migration fallback value (0) vs this runtime initialization value (1). The distinction is intentional: schema default applies to fields absent in legacy files; runtime initialization applies to newly-detected topics. |
