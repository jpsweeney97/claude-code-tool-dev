...
**Serializer contract:** Implementations MUST use `json.dumps` or equivalent serializer that preserves list insertion order for `pending_facets`. Do NOT use `sort_keys=True` or any serializer that normalizes array element order. Write-time assertion: the serialized JSON array order MUST match the in-memory insertion order. See [data-model.md#registryseed](data-model.md#registryseed) for the persistence-layer surface of this constraint.
...
|-----------|----------------|
| `absent → detected` | `first_seen_turn` ← current turn, `last_seen_turn` ← current turn, `consecutive_medium_count` ← (1 if medium AND leaf-kind, else 0) (family-kind topics always initialize `consecutive_medium_count` ← 0 regardless of confidence), `coverage_target` ← from `ClassifierResult.resolved_topics[].coverage_target`, `facet` ← from `ClassifierResult.resolved_topics[].facet`. See [data-model.md defaults table](data-model.md#schema-evolution-constraint) for the schema migration fallback value (0) vs this runtime initialization value (1). The distinction is intentional: schema default applies to fields absent in legacy files; runtime initialization applies to newly-detected topics. |
...
| `contradicts_prior`, `prescriptive`, or `extends_topic` hint resolves to `detected` topic | No field update — scheduling effect only (elevated to materially new for immediate lookup) |
| `contradicts_prior`, `prescriptive`, or `extends_topic` hint resolves to `deferred` topic | Transition to `detected` first: `state` ← `detected`, `deferred_reason` ← null, `deferred_ttl` ← null, `last_seen_turn` ← current turn (applying `deferred → detected` field update rules). Then enter the lookup path from `detected`. Consistent with scheduling effects table rows for these hint types. |
...
| `deferred → detected` (high-confidence bypass, via [scheduling step 2](#scheduling-rules)) | `state` ← `detected`, `deferred_reason` ← null, `deferred_ttl` ← null, `last_seen_turn` ← current turn, `consecutive_medium_count` ← 0 (this row fires only on high-confidence re-detection — the medium-confidence conditional from the TTL-expiry row does not apply here), `coverage_target` ← from `ClassifierResult.resolved_topics[].coverage_target`, `facet` ← from `ClassifierResult.resolved_topics[].facet` (both updated to current classifier resolution, identical to TTL-expiry row) |
...
| `deferred → deferred` (TTL=0, topic absent from classifier) | `deferred_ttl` ← `injection.deferred_ttl_turns` from config (reset). `deferred_reason` ← unchanged (retained from original deferral). `last_seen_turn` is NOT updated — the topic was absent from classifier output this turn. `state` unchanged. `consecutive_medium_count` ← 0 (via the general "topic absent from classifier output" rule in the row above — not a transition-specific action; listed here for completeness). **Staleness note:** `coverage_target` and `facet` retain their values from the original detection. These fields are not refreshed until the topic reappears in classifier output (at which point `deferred → detected` fires and both fields update from the current `ClassifierResult`). Between the original detection and re-detection, the stored values may refer to a stale classifier resolution. Consumers of `coverage_target` or `facet` during `deferred` state MUST treat these values as potentially stale and re-validate against current classifier output before using them for packet building. The fields are authoritative only at detection time — between detection and re-detection, they may refer to a superseded classifier resolution. |
...
2. **Materially new** = one of:
   ...
   - Agent provides `semantic_hints` (see [below](#semantic-hints)) referencing a claim that touches a detected topic
...
**Steps 1–5 and 7–9 run in the CLI via `dialogue-turn`.** Step 6 (scout priority) and Step 10 (target-match) run agent-side — the CLI has no scout awareness and no target-match input. See [integration.md#mid-dialogue-phase-per-turn-in-codex-dialogue](integration.md#mid-dialogue-phase-per-turn-in-codex-dialogue) for the agent-side flow.
...
| `prescriptive` | `injected` | No effect — topic already covered. **Replay fixture assertion:** `final_registry_file_assertions` MUST verify: (a) `pending_facets` array is unchanged from initial state, (b) no injection or deferral mutations occur, (c) `state` remains `injected`, (d) `last_seen_turn` equals the current turn (confirms the hint was processed, not silently dropped). |
...
**Intra-turn hint ordering:** When multiple hints are provided for the same topic in a single `--semantic-hints-file`, they are processed in array order. `contradicts_prior` mutations to `pending_facets` are visible to subsequent `extends_topic` cascade resolution within the same turn (step 2 of the `extends_topic` cascade reads `pending_facets` as mutated by prior hints in this turn, not as of turn-start). For hints targeting different topics, processing order does not matter — per-topic state is independent.
...
| Staged packet doesn't match composed follow-up target | [Target-match predicate](integration.md#target-match-predicate) (scheduling step 10, agent-side, invoked during turn-loop step 5.5) | Defer CCDI candidate (→ `deferred: target_mismatch`) via `--mark-deferred` |
