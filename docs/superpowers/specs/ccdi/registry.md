...
**Serialization invariant:** All fields MUST be written to the registry file including null-valued ones — see [data-model.md#registryseed](data-model.md#registryseed) for the normative requirement. An absent field triggers entry reinitialization per the resilience principle.
...
| `extends_topic` hint resolves to `injected` topic | No field update — `facet_expansion` candidate emitted immediately this turn via [scheduling step 10](#scheduling-rules). `pending_facets` is only mutated by `contradicts_prior` hints (see row above). State stays `injected`. |
...
| `suppressed → detected` (re-entry, any `suppression_reason`) | `state` ← `detected`, `suppression_reason` ← null, `suppressed_docs_epoch` ← null, `last_seen_turn` ← current turn, `coverage_target` ← from `ClassifierResult.resolved_topics[].coverage_target`, `facet` ← from `ClassifierResult.resolved_topics[].facet` (refreshed from current classifier output, consistent with `deferred → detected`). This row applies identically for both `weak_results` and `redundant` suppression reasons — the field resets are the same regardless of reason. |
| `deferred → detected` (TTL expiry + reappearance) | `state` ← `detected`, `deferred_reason` ← null, `deferred_ttl` ← null, `last_seen_turn` ← current turn, `consecutive_medium_count` ← (1 if re-entry turn confidence is medium, else 0), `coverage_target` ← from `ClassifierResult.resolved_topics[].coverage_target`, `facet` ← from `ClassifierResult.resolved_topics[].facet` (both updated to current classifier resolution) |
| `deferred → deferred` (TTL=0, topic absent from classifier) | `deferred_ttl` ← `injection.deferred_ttl_turns` from config (reset). `last_seen_turn` is NOT updated — the topic was absent from classifier output this turn. `state`, `deferred_reason` also unchanged — only `deferred_ttl` is modified. Note: The "absent from classifier output" reset rule also applies — `consecutive_medium_count` is reset to 0 during this transition since the topic was not in classifier output. |
...
7. **Target-match check (agent-side — not CLI):** After building a packet for a scheduled candidate, the agent verifies the packet supports the composed follow-up target. The agent performs this check. When condition (a) fails, the agent MUST invoke `classify --text-file <follow-up>` and evaluate condition (b) before deciding to defer. The agent makes the final pass/fail decision and invokes `build-packet --mark-deferred <topic_key> --deferred-reason target_mismatch --skip-build` if the check fails. A packet is **target-relevant** when at least one of: (a) any of the packet's `topics` (from the `<!-- ccdi-packet -->` metadata comment) appears as a substring (case-insensitive) in the composed follow-up text, OR (b) running `classify` on the composed follow-up text produces a topic that overlaps with the packet's `topics`. The **composed follow-up target** is the follow-up question text that `codex-dialogue` has composed for the current turn (exists after composition, before CCDI PREPARE). See [integration.md#target-match-predicate](integration.md#target-match-predicate) for the CLI interface (`--mark-deferred`, `--skip-build`).
8. **Schedule** highest-priority materially new topic for lookup. **Priority ordering among candidates:** (1) `confidence` descending (`high` before `medium`; `null` for hint-driven candidates sorts after `medium`), (2) `first_seen_turn` ascending (earlier-detected topics first), (3) `topic_key` lexicographic ascending (deterministic tiebreaker). This ordering ensures identical scheduling decisions for identical inputs regardless of implementation, per the CLI determinism principle in [foundations.md#cliagent-separation](foundations.md#cliagent-separation).
9. **`pending_facet` candidate emission:** For each `injected` topic with non-empty `pending_facets` (populated by prior `contradicts_prior` hints — `extends_topic` does NOT write to `pending_facets`), emit a `pending_facet` candidate at the first pending facet not already in `facets_injected`. This check runs on every `dialogue-turn` call, independent of classifier output.
10. **`facet_expansion` candidate emission:** When an `extends_topic` hint resolves to an `injected` topic at a facet not yet in `facets_injected`, emit a `facet_expansion` candidate at the cascade-resolved facet (hint-resolved → `pending_facets[0]` → `default_facet`). If all candidate facets are already in `facets_injected`, discard the hint silently — no candidate emitted, no state change.
