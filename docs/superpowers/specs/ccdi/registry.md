...
| `extends_topic` | `injected` | Flag for facet expansion via [scheduling step 10](#scheduling-rules). Cascade: (1) facet resolved from `claim_excerpt` classification, (2) `pending_facets[0]` if resolved facet already in `facets_injected`, (3) `default_facet` if all pending facets exhausted. Emit a `facet_expansion` candidate at the first cascade facet not yet in `facets_injected`. If all candidate facets (resolved, pending, default) are already in `facets_injected`: discard hint silently, no state change, no candidate emitted. Does NOT mutate `coverage.pending_facets` — only `contradicts_prior` writes to that field. |
