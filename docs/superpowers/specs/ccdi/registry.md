...
| `[looked_up] → suppressed` | `state` ← `suppressed`, `suppression_reason` ← reason, `suppressed_docs_epoch` ← current pinned inventory snapshot's `docs_epoch` (from `--inventory-snapshot`, see [integration.md#cli-tool-topic_inventorypy](integration.md#cli-tool-topic_inventorypy)) |
...
| Registry file loaded with `results_file` field present | CLI load-time check | Strip `results_file` from the in-memory representation and log warning. The `results_file` is a transient field written by `ccdi-gatherer` into the RegistrySeed sentinel block (see [data-model.md#registryseed](data-model.md#registryseed)) and consumed by the agent during the initial CCDI commit phase. Its presence at load time indicates abnormal shutdown. The stripped state is written back on the next normal mutation. |
