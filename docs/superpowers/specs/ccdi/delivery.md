...
**Freshness guardrail test:** A separate integration test file (`test_shadow_freshness_guardrail.py`, not the replay harness) MUST verify the guardrail fires correctly. The test invokes `validate_graduation.py` against a `graduation.json` (which MUST include the `shadow_adjusted_yield` field as defined in the [graduation schema](#graduation-protocol)) where `shadow_adjusted_yield` was computed from stale classify results (same `classify_result_hash` across all turns for a topic with `shadow_defer_intent`). Assert: validator emits a warning that the freshness guardrail is not satisfied and does not use `shadow_adjusted_yield` as the graduation gate. The replay harness (Layer 2a) does not invoke `validate_graduation.py` — freshness guardrail verification requires the validator, which is an integration test concern.
