...
| CLI backstop: `build-packet --shadow-mode --mark-deferred scout_priority` zero registry mutations | **Observable assertion:** JSON-parse semantic identity — load registry file before and after `build-packet --shadow-mode --mark-deferred scout_priority`, parse both as JSON, assert `json.loads(before) == json.loads(after)`. This detects no-op rewrites that would change file mtime but not content. **Layer note:** This test exercises CLI-deterministic behavior (direct `build-packet` invocation with no agent involvement). It is placed in Layer 2b because the authority source is integration.md's `behavior_contract` (pipeline isolation invariant), but the test itself can be implemented as a CLI integration test (Layer 1/CLI). For Phase A, implement this test at the CLI integration level to avoid dependency on Layer 2b infrastructure. Add a companion Layer 2b test verifying the *agent* passes `--shadow-mode` when graduation status is not approved. |
...
| `intra_turn_hint_ordering.replay.json` | Single `dialogue-turn` call with two hints in the `--semantic-hints-file`: (1) `contradicts_prior` on an injected topic T, adding facet F to `pending_facets`; (2) `extends_topic` on the same topic T where F is the cascade-resolved facet (i.e., F is in `pending_facets` after hint 1 is processed). Assert: `dialogue-turn` produces a candidate with `candidate_type: "pending_facet"` and `facet: F`, proving hint 1 was processed before hint 2 (sequential processing order). `final_registry_file_assertions` MUST include: `{"path": "T.pending_facets", "equals": [F]}` after `dialogue-turn` completes (before any `build-packet` commit clears it). |
