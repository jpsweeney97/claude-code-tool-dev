...
**Status enum (closed):** Valid `status` values are `"approved"` and `"rejected"` only. Unknown values MUST be rejected by the schema validator (not silently treated as shadow mode). The shadow-mode gate's catch-all `else → shadow mode` handles runtime resilience but does not imply the enum is open.
...
| `shadow_suppressed` field presence | For every `ccdi_trace` entry where `action != "shadow_defer_intent"`, assert `shadow_suppressed` key is present (boolean). Per [integration.md#ccditrace-output-contract](integration.md#ccditrace-output-contract). **Note on `shadow_suppressed` and key count:** `shadow_suppressed` is described as the "9th field, not counted in the 8-key invariant for backward compatibility" in integration.md. However, delivery.md requires its presence via test assertion for all entries where `action != 'shadow_defer_intent'`. The 8-key invariant is the contractual floor; `shadow_suppressed` is a mandatory 9th field above that floor. Implementations MUST include `shadow_suppressed` in all per-turn trace entries. The key count name ("8-key") is retained for identification only — the effective required key count is 9. |
