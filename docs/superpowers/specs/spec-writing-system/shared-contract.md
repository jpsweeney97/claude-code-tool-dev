...
No `review_cluster` field — structural roles are derived from claims by both skills using the shared derivation table ([Claim-to-Role Derivation Table](#claim-to-role-derivation-table)).
...
- `reference` = file has zero effective claims. Only valid for non-normative files (see normative claim rule in [File Frontmatter](#file-frontmatter)).
