...
- `work_max_creates` validation (VR-3A-15): Set `work_max_creates: 0` and invoke ticket create → assert rejection with `"work_max_creates must be >= 1"` before any file I/O. Repeat with `-1`. Test string form `"0"` for type coercion. Mirrors [VR-3A-11](#required-verification-4) for `knowledge_max_stages`. (SY-20)
