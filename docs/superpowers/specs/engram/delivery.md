...
- `work_max_creates` validation (VR-3A-15): Set `work_max_creates: 0` and invoke ticket create → assert rejection with `"work_max_creates must be >= 1"` before any file I/O. Repeat with `-1`. Test string form `"0"` for type coercion. Mirrors [VR-3A-11](#required-verification-3) for `knowledge_max_stages`. (SY-20)
...
- Chain state non-creation when session_id is None (VR-4A-27): Patch `session_id` resolution to return `None`. Invoke `/load`. Assert: (a) no chain state file created in `chain/`; (b) diagnostic warning logged; (c) subsequent `/save` snapshot does not contain `resumed_from` field. (SY-25)
...
- `/quicksave` direct-write tool path (VR-4A-29): Instrument `/quicksave` execution. Assert: the skill invokes the Write tool (not Bash) to create the checkpoint file. Alternative method: mock the Bash tool to raise on invocation; run `/quicksave`; assert checkpoint created without triggering Bash mock. This ensures the [direct-write path](enforcement.md#direct-write-path-authorization) is taken, not a Bash engine route. (SY-29)
- Text search result ordering (VR-4A-30): Create fixtures with 3 entries in each subsystem at different `created_at` timestamps. Run `query(text="<match>")`. Assert: within each subsystem group, results are ordered by `created_at` descending. (SY-30)
