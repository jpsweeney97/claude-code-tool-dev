...
- **VR-NEW-8 (AuditEntry version handling):** JSONL shard with three `AuditEntry` entries — `schema_version: "1.0"` (current), `"2.0"` (future major), `"1.1"` (same-major minor). Assert reader returns entries 1 and 3; entry 2 skipped without exception, warning in `QueryDiagnostics.warnings`.
