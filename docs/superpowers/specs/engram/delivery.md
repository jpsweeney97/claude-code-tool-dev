...
- work_dedup_fingerprint path normalization (VR-3A-22): Submit two `DeferEnvelope`s with `source_file` paths that differ only by normalization (e.g., `./src/foo.py` vs `src/foo.py`). Assert both produce the same `work_dedup_fingerprint`.
