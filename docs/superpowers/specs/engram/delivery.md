...
- `.diag` unknown schema_version handling (VR-4A-31): Construct a `.diag` file with two entries — one with `schema_version: "1.0"`, one with `schema_version: "99.0"`. Run `/triage`. Assert: entry with `"99.0"` treated as opaque (logged but not interpreted); entry with `"1.0"` interpreted normally. (SY-60)
