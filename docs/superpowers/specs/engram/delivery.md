...
- **VR-NEW-7 (cross-worktree append merge):** Append two independent `lesson-meta` entries in parallel to separate worktree copies of `learnings.md`. Confirm clean merge (no conflict markers). (Optional integration test.)
...
- Twice-per-reader scan count (VR-4A-32): Instrument `NativeReader.scan()` with a call counter. Run `query()` against a fixture with files in both shared and private roots. Assert: each reader's `scan()` is called exactly twice — once with `root_type="shared"`, once with `root_type="private"`. (SY-61)
...
- save_recovery.json success-path write (VR-4A-34): Run `/save` where all sub-operations succeed. Assert: `save_recovery.json` exists with `results.snapshot.status == "ok"`, `results.defer.status == "ok"`, `results.distill.status == "ok"`. Assert: `"schema_version"` key is absent from the written `save_recovery.json`. (SY-76)
...
- VR-5-5: All skills in the progressive activation manifest pass their smoke test (final progressive gate)
