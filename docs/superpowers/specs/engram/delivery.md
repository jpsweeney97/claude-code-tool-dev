...
- **VR-NEW-9 (RecordMeta version handling):** Context snapshot and Work ticket with `RecordMeta.schema_version: "2.0"` — assert reader skips entry per-entry with warning in `QueryDiagnostics.warnings`, entries with `"1.0"` and `"1.1"` are returned.
