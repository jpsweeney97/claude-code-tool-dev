...
- Normalization boundary test (VR-0A-6): construct a string where `knowledge_normalize` and `drift_hash`-level normalization (NFC+LF only) produce different outputs (e.g., trailing whitespace). Assert `content_hash(input) != drift_hash(input)`. Separately, assert `content_hash(input) != work_dedup_fingerprint(input, [])` for a mixed-case input. This proves the three pipelines diverge — using the wrong one would produce a different hash. Trailing newline normalization: assert that `knowledge_normalize()` applied to content ending with no newline, one newline, and multiple newlines all produce a document ending with exactly one `\n`, and the resulting `content_sha256` is identical for all three inputs.
- Fence-aware `knowledge_normalize` tests (VR-0A-10): (a) input with trailing whitespace inside a fenced code block (triple backtick) → assert trailing whitespace preserved (rules 3–6 suspended inside fences); (b) input with 2+ consecutive blank lines inside a fence → assert blank lines preserved (rule 5 suspended); (c) nested fence: triple-backtick block inside quadruple-backtick block → assert inner content treated as code (not a fence boundary); (d) unclosed fence at end-of-file → assert all content after opening fence treated as inside the fence; (e) fence with info string (e.g., `` ```python ``) → assert fence detected and rules 3–6 suspended inside.
...
- **VR-NEW-7 (cross-worktree append merge):** Append two independent `lesson-meta` entries in parallel to separate worktree copies of `learnings.md`. Confirm clean merge (no conflict markers). Required at Step 2a exit gate. Environment: requires two independent git worktrees from the same repo.
- **VR-0A-18 (slash in record_id):** `RecordRef.from_str("work/ticket/T-2026-03/01", repo_id)` → assert `subsystem="work"`, `record_kind="ticket"`, `record_id="T-2026-03/01"`. Verifies split-on-first-two-slashes rule.
- content_sha256 golden-value test (VR-0A-14): Construct a two-entry `learnings.md` fixture with known content. Compute `content_hash()` on the specified byte range. Assert result equals a golden hex value embedded in the test. This locks the byte-range interpretation. Add parametric boundary tests: (a) content ending with trailing blank lines — assert `content_hash` excludes them per the byte-range rule, (b) content with exactly one trailing newline — assert stable hash, (c) content with no trailing newline — assert `knowledge_normalize` adds one before hashing.
...
- RecordRef construction-time validation (VR-0A-16): (a) `RecordRef(subsystem="")` → assert `ValueError` (empty string is always invalid). (b) `RecordRef(subsystem="invalid")` → assert `ValueError` — deferred to the step when the allowed subsystem value set is resolved; test stub must exist with a TODO comment citing that step.
- **VR-NEW-9 (RecordMeta version handling):** Context snapshot and Work ticket with `RecordMeta.schema_version: "2.0"` — assert reader skips entry per-entry with warning in `QueryDiagnostics.warnings`, entries with `"1.0"` and `"1.1"` are returned. Include a test case with `RecordMeta.schema_version: "staged"` — assert reader routes to staging-specific handling, not same-major tolerance parsing. Include a Knowledge-subsystem test fixture: `lesson-meta` with `meta_version: "2.0"` (future major). Assert `RecordMeta.schema_version` derivation from `lesson-meta.meta_version` produces `"2.0"`. Assert reader skips entry per same-major tolerance rule with warning.
- `collect_trust_triple_errors()` stable error strings (VR-0A-17): For each of the three validation rules, call `collect_trust_triple_errors()` with a value that triggers that rule. Assert the returned list contains exactly the specified stable error string, including `!r` formatting: (a) `collect_trust_triple_errors(False, "user", "sid")` → `['hook_injected: must be True, got False']`; (b) `collect_trust_triple_errors(True, "invalid", "sid")` → `["hook_request_origin: must be one of {'user', 'agent'}, got 'invalid'"]`; (c) `collect_trust_triple_errors(True, "user", "")` → `["session_id: must be non-empty string, got ''"]`. See [types.md §Trust Validation](types.md#trust-validation). (SY-22)
...
- `engram init` idempotency matrix (VR-0B-1): (a) `.engram-id` absent → creates file with valid UUIDv4, stages for commit; stdout contains a `git commit` command string referencing `.engram-id`; (b) `.engram-id` present with valid UUID → no-op, exits 0, prints "already initialized"; (c) `.engram-id` present with malformed content → error with message instructing manual repair or re-init with `--force`; (d) `engram init --force` with malformed `.engram-id` → overwrites file with valid UUIDv4, exits 0 (SY-45); (e) `engram init --force` with a valid `.engram-id` → assert the command refuses. Exit code: 1 (user error). The warning text must match exactly: `"this will change repo_id; use --force --yes to confirm"`. `engram init --force --yes` with a valid `.engram-id` → assert the file is overwritten with a new UUIDv4.
...
- Bridge test additionally verifies old engine accepts converted JSON (VR-1-1): call old ticket engine ingest with bridge output, assert non-error response. This test requires the old ticket engine to be available (bridge period). It must be executed during Step 1 before the bridge is removed. After Step 3a (bridge replacement), this test is no longer executable — mark as bridge-only in the test suite.
- SourceResolver failure path (VR-1-3): fixture with snapshot missing `session_id` in frontmatter → assert SourceResolver raises with explicit error referencing source snapshot path. Do not accept empty-string return — see [normative requirement above](#step-1-bridge-cutover).
...
- Bridge field-exactness (VR-1-4): Assert the converted JSON from the bridge adapter contains exactly the expected fields and no additional ones. This catches accidental field additions to `EnvelopeHeader` that the bridge adapter fails to filter. (SY-78)
...
- Write concurrency test (VR-2A-3): Spawn two threads, each calling the Knowledge engine publish path simultaneously. Assert: both entries appear in `learnings.md` without corruption and the lock file is absent post-operation. Timeout test: hold the lock externally (via a subprocess — `fcntl.flock` locks are per-open-file-description; threads within the same process share file descriptors and cannot block each other) and assert publish returns error with `"learnings.md is locked by another operation"` message within 5 seconds.
...
- `/curate` dedup-within-lock TOCTOU property (VR-2A-6): Instrument the Knowledge engine publish path to record lock-acquisition and dedup-check ordering. Assert: dedup check occurs after lock acquisition and before write. Alternative: hold the lock externally (via a competing thread), then call the publish path with a candidate whose `content_sha256` matches a file published by the lock-holder mid-race. Assert: the publish either blocks until lock release and then correctly deduplicates, or returns the existing entry — never produces a duplicate. (SY-24)
...
- Staging-meta sort_keys=True determinism (VR-2A-8): Submit same `DistillCandidate` twice. Assert staging-meta JSON comment is byte-identical.
- Engine content_sha256 hash verification (VR-2A-9): Submit a `DistillEnvelope` where `content_sha256` does not match `content_hash(content)`. Assert the Knowledge engine rejects the candidate with a hash-mismatch error.
- Promote-meta cross-validation (VR-2A-10): After a successful promote, read `promote-meta` and assert `promoted_content_sha256 == content_hash(DistillCandidate.content)` (the content field from the original distill step) and `lesson_id` matches the promoted lesson's `RecordRef`.
...
- Staging body-identity invariant (VR-2A-11): After staging a lesson via `/distill`, read the staging file. Assert the markdown body is byte-identical to `DistillCandidate.content` and `content_hash(body) == content_sha256`.
...
- Trust triple call-site for Knowledge (VR-2A-NEW-1): Assert `collect_trust_triple_errors()` is invoked at every Knowledge mutating entrypoint (publish, staging write, promote-meta write) using the same dual-method approach as VR-3A-9. Knowledge entrypoints ship at Step 2a — this verification must not be deferred to Step 3a.
- Knowledge .engram-id ordering (VR-2A-NEW-2): For each Knowledge mutating entrypoint, invoke with `.engram-id` absent. Assert the entrypoint returns the initialization error *before* attempting trust-triple validation. Verify ordering: `.engram-id` check → trust triple validation.
- Staging content divergence warning (VR-2A-NEW-3): After staging a lesson via `/distill`, manually modify the staging file's JSON `content` field (without changing the markdown body). Run `/curate`. Assert a per-entry warning in `QueryDiagnostics.warnings` for the body-vs-JSON divergence. Assert `IndexEntry.snippet` is derived from the markdown body (not the modified JSON `content`).
- Concurrent promote-meta uniqueness (VR-2A-NEW-4): Invoke two concurrent `/promote` calls for the same `lesson_id`. Assert exactly one succeeds and one either (a) detects the existing `promote-meta` via Branch B1 rejection, or (b) blocks on `flock` and completes after the first — but in no case are two `promote-meta` entries written for the same `lesson_id`.
- Normalizer call-site audit (VR-2A-NEW-5): Mock each normalizer (`knowledge_normalize`, `work_normalize`, NFC+LF) to return a tagged output that identifies which normalizer ran. For each hash call-site (`content_hash`, `drift_hash`, `work_dedup_fingerprint`), assert the correct normalizer was invoked. A swapped normalizer produces different hashes silently — this call-site audit catches the swap.
- Staged snippet source verification (VR-2A-NEW-6): Stage a lesson where the markdown body and JSON `content` field have different first 200 characters (by manually modifying the JSON after staging). Assert `IndexEntry.snippet` is derived from the markdown body, not the JSON field. This verifies the 'MUST NOT drive display output' constraint on the JSON `content` field.
- Co-deployment invariant at activation (VR-2A-NEW-9): Invoke the promote script with `scripts/` directory absent from the plugin root. Assert promote fails with a diagnostic referencing the missing directory. See [co-deployment invariant](enforcement.md#step-1-injection-pretooluse). This test activates at Step 2a (when `engine_trust_injection` first requires co-deployment), not Step 5.
...
- Compatibility harness pass threshold (VR-3A-6): 100% of ported compatibility-critical fixtures must match on response envelope, on-disk ticket output, and audit side effects. Harness exceptions (intentional behavioral differences, e.g., new dedup logic) capped at 5, each requiring a comment explaining the intentional divergence. Assert: `len(harness_exceptions) <= 5`.
...
- Trust injection path matching negative test (VR-3A-8): invoke `engram_guard` with a Bash tool call executing `python3 /tmp/engine_work.py` (valid filename, outside `<engram_scripts_dir>`). Assert: [payload file](enforcement.md#payload-file-contract) is NOT created. Then invoke with `python3 <engram_scripts_dir>/engine_work.py` (correct path). Assert: payload file IS created with valid trust triple fields. `<engram_scripts_dir>` in the test context: derive from the test's known plugin root path, or patch `engram_guard.__file__` to point to a fixture directory containing a known `scripts/engine_work.py`.
- Trust triple call-site completeness (VR-3A-9): for each documented mutating entrypoint (Work: ticket creation, ticket update, ticket close; Knowledge: knowledge publish, staging write, promote-meta write), assert via AST scan or instrumented test that [`collect_trust_triple_errors()`](enforcement.md#step-2-validation-engine-entrypoint) is called before any filesystem write. Acceptable methods: (a) `ast.parse` + visitor asserting the call appears before `open(..., 'w')` / `os.replace` / `shutil` calls; (b) mock `collect_trust_triple_errors` to raise on first call, invoke entrypoint, assert exception propagated. Context engine scripts must **not** call `collect_trust_triple_errors()` — assert zero matches via grep. The test suite MUST exercise both method (a) and method (b) across all 6 mutating entrypoints. Method (a) — AST scan — is applicable only to single-function entrypoints where both calls appear in the same function body. For class-based or OOP entrypoints, use method (b) exclusively. If no single-function entrypoints exist, method (b) covers all entrypoints and the dual-method requirement is satisfied by exhaustive method (b) coverage. If method (a) is never used (all entrypoints are class-based), the test file must document this decision: "All 6 mutating entrypoints use class-based architecture — method (a) AST scan not applicable."
...
- Payload containment check (VR-3A-13): Invoke `engram_guard` with a legitimate engine pattern but a crafted payload path that resolves outside `.claude/engram-tmp/` (e.g., via `../` traversal). Assert: exit code 2, diagnostic message contains `"engram_guard: payload path outside containment boundary"`. Then invoke with a valid payload path inside `.claude/engram-tmp/`. Assert: exit code 0, payload file created. This is a security-critical path — see [payload file contract](enforcement.md#payload-file-contract). (SY-64)
...
- Origin-matching call-site completeness (VR-3A-14): For each mutating Work engine entrypoint (`_user.py` pattern), assert that the implementation checks `hook_request_origin == "user"` and rejects `"agent"` after `collect_trust_triple_errors()` succeeds. For each agent-initiated entrypoint (`_agent.py` pattern), assert the reverse. Knowledge engine entrypoints: (a) publish entrypoint (expected origin: `"user"`); (b) staging write entrypoint `/distill` (expected origin: `"agent"`); (c) promote-meta write entrypoint (expected origin: `"user"`). Acceptable methods: AST scan for the comparison expression, or instrumented test submitting a valid triple with the wrong origin and asserting rejection. Additionally, assert the error message matches the stable format: `'hook_request_origin: expected {expected!r} for this entrypoint, got {actual!r}'`. A regex match on the format template is sufficient — the assertion must verify both the field name and the quoting pattern, not just that rejection occurred. Additionally, for each mutating entrypoint, assert `validate_origin_match` is called by name (via AST scan or mock-and-assert — parallel to VR-3A-9 method (b) for `collect_trust_triple_errors()`). An inline reimplementation that produces the same output but does not call `validate_origin_match` must fail this test. (SY-39)
- `work_max_creates` validation (VR-3A-15): Set `work_max_creates: 0` and invoke ticket create → assert rejection with `"work_max_creates must be >= 1"` before any file I/O. Repeat with `-1`. Test string form `"0"` for type coercion. Mirrors VR-3A-11 (see Step 3a Required Verification above) for `knowledge_max_stages`. (SY-20)
...
- AuditEntry session_id invariant (VR-3A-20): Assert that an `AuditEntry` with `trust_triple_present: false` has `session_id: null`. Assert that an `AuditEntry` with `trust_triple_present: true` has `session_id` matching the session's UUID.
- DeferEnvelope context=None idempotency (VR-3A-21): Submit a `DeferEnvelope` with `context=None`. Assert the Work engine creates a ticket with no context field. Submit the identical envelope again — assert idempotency (no duplicate ticket).
- work_dedup_fingerprint path normalization (VR-3A-22): Submit two `DeferEnvelope`s with `key_file_paths` paths that differ only by normalization (e.g., `./src/foo.py` vs `src/foo.py`). Assert both produce the same `work_dedup_fingerprint`.
...
- **VR-NEW-8 (AuditEntry version handling):** JSONL shard with three `AuditEntry` entries — `schema_version: "1.1"` (current), `"2.0"` (future major), `"1.2"` (same-major minor). Assert reader returns entries 1 and 3; entry 2 skipped without exception, warning in `QueryDiagnostics.warnings`.
...
- **VR-3A-NEW-1 (engine payload containment defense-in-depth):** Engine invoked with payload path pointing to a file outside `.claude/engram-tmp/` (e.g., `../injected-trust.json`). Assert engine rejects with containment diagnostic before attempting trust-triple validation.
...
**Intra-step ordering:** Within Step 4a, ALL hooks must be deployed and validated before ANY skill is activated:
1. `engram_session` startup test must pass before any skill invocation
...
- SessionStart timing (VR-4A-3): Additional archived snapshot TTL test: add (a) 10 archived snapshots in `.archive/` older than 90 days and (b) 3 younger than 90 days. Assert old deleted, young retained, combined cleanup respects 50-file cap. Full test: run `engram_session` against fixture with 200 snapshots (realistic 90-day max at 2/day), 20 chain files; assert **median** wall-clock < 500ms over 5 runs. Environment qualifier: local filesystem with reasonable I/O latency. If per-file cleanup exceeds 5ms, abort remaining cleanup with warning rather than blocking. Cap enforcement: 100 expired snapshots → exactly 50 deleted, 50 remain. Run again → 50 more deleted. Environment probe: before asserting <500ms, measure median per-file read latency on a 10-file fixture. If per-file latency exceeds 10ms, mark the timing assertion as skipped/environment with a warning. (d) Orphan payload cleanup (SY-28): create 5 payload files in `.claude/engram-tmp/` with timestamps >24h old and 3 with timestamps <24h. Run `engram_session`. Assert: old files deleted, young files retained. Cap enforcement: 25 orphan files (>24h) → exactly 20 deleted per session (cap from [SessionStart hook table](enforcement.md#sessionstart-hook)). Cleanup failure (permission denied) → fail-open (session proceeds, warning logged). (SY-34) The environment probe (per-file latency > 10ms → skip timing assertion) is a precondition check executed before the 200-file test, not a conditional within it. This test is marked `@pytest.mark.slow` and excluded from default CI runs. Separate the timing assertion: (a) The 500ms *behavioral* requirement (cleanup runs without blocking) is verified in standard CI via a mock filesystem. The mock behavioral test must verify: (1) per-file abort logic fires (mock one file's cleanup to take >5ms, assert remaining files in that category are skipped), (2) elapsed-time guard fires (mock elapsed > 450ms after first optional operation, assert remaining operations skipped). Without these assertions, the mock test verifies nothing about timing behavior. (b) The 500ms *wall-clock* measurement is a performance test appropriately marked `@pytest.mark.slow`. The behavioral test verifies the algorithm; the slow test measures it on real hardware. **Stability note:** This test measures timing-dependent behavior. Use a generous timeout (10x expected duration) and allow 1 retry before failure. If flaky in CI, investigate whether the CI runner's filesystem introduces non-deterministic latency.
...
- engram_quality catch-all exception test (VR-4A-14): monkey-patch the hook's internal validation function to raise `RuntimeError`. Assert: exit code 0, `[engram_quality:error]` log entry present. Verifies the outermost catch-all, not just specific failure paths. (SY-62)
...
- **VR-4A-38 (engine+hook co-producer concurrency):** Spawn two producers simultaneously: (a) an engine producer appending `defer_completed` with payload and (b) a hook producer appending a `snapshot_written` event. Assert: both entries appear in shard, no partial writes, correct JSON per line, `producer` field distinguishes them.
...
- Context direct-write path authorization (VR-4A-19): Capability-scoped test matrix using canonical absolute paths (not tilde-relative). The `context_direct_write_authorization` capability state is a test fixture parameter.
  ...
  (a) Write to `Path(os.path.expanduser("~")) / ".claude" / "engram" / <correct_repo_id> / "snapshots" / "test.md"` → assert allowed (branch 2)
  (b) Write to `Path(os.path.expanduser("~")) / ".claude" / "engram" / <correct_repo_id> / "checkpoints" / "test.md"` → assert allowed (branch 2)
  (c) Write to `Path(os.path.expanduser("~")) / ".claude" / "engram" / <different_repo_id> / "snapshots" / "test.md"` → assert allowed via branch 4 (path is outside this repo's Context root, falls through to allow)
  (d) Write to `Path(os.path.expanduser("~")) / ".claude" / "engram" / <different_repo_id> / "checkpoints" / "test.md"` → assert allowed via branch 4
  ...
  (j) Tilde expansion: tilde-relative paths must match canonical absolute paths after `expanduser()` — see [path canonicalization](enforcement.md#protected-path-enforcement)
...
- Session diagnostic channel integration (VR-4A-20): (a) Force `engram_register` to fail (make shard file unwritable). Assert `.diag` created with JSONL entry containing `schema_version`, `hook`, `failure_type`, `ts` fields. (b) Run `/triage` for that session. Assert output surfaces `"ledger unavailable in session <session_id>"`. (c) Assert other sessions unaffected.
- Enforcement Boundary Constraint structural test (VR-4A-23): Dynamically derive the PostToolUse hook list from `settings.json` (or the hook registration source). AST scan of all registered PostToolUse hook files — assert no path returns exit code 2 (Block). Grep patterns: `exit(2)`, `sys.exit(2)`, `return {"decision": "block"}`, `"action": "block"`. Assert every PostToolUse hook file is included in the AST scan. A new PostToolUse hook added to `settings.json` but not to the scan triggers a test failure. This verifies the [enforcement boundary constraint](foundations.md#enforcement-boundary-constraint-invariant) structurally. (SY-4)
- `snapshot_written` per-producer emission (VR-4A-24): For each snapshot producer, run the skill against a fixture, then inspect the ledger shard for that session. Assert: (a) `/save` → `snapshot_written` entry with `orchestrated_by: "save"` present; (b) `/quicksave` → `snapshot_written` entry with `orchestrated_by: "quicksave"` present; (c) `/load` (archive path) → `snapshot_written` entry with `orchestrated_by: "load"` present. Use distinct session fixtures to isolate producers. (SY-5)
- Guard decision branch ordering (VR-4A-25): Construct fixtures that exercise branch priority under all capability combinations: (a) path within `~/.claude/engram/<repo_id>/snapshots/` (Context branch 2 match) and NOT in the protected-path table → assert allowed via branch 2; (b) path within `engram/knowledge/` (branch 3 match) → assert blocked; (c) engine Bash invocation targeting a Context path → assert it hits branch 1 (allowed with payload), not branch 3 (blocked). These three cases verify branch ordering holds with all guard capabilities active. (SY-6)
- `operation_id` non-self-generation (VR-4A-26): Run `/save` against a fixture. Collect all ledger entries from that session shard. Assert: all entries where `operation_id` is non-None share the same UUID. Negative test: mock an engine to generate its own UUID, assert the resulting ledger entries have mismatched `operation_id` values (proving the test would catch self-generation). (SY-21)
- Chain state non-creation when session_id is None (VR-4A-27): Patch `session_id` resolution to return `None`. Invoke `/load`. Assert: (a) no chain state file created in `chain/`; (b) diagnostic warning logged; (c) subsequent `/save` snapshot does not contain `resumed_from` field. (SY-63)
- Recovery manifest format and retry (VR-4A-28): Simulate `/save` partial failure (mock distill engine to fail). Assert: (a) `save_recovery.json` written to `~/.claude/engram/<repo_id>/`; (b) `snapshot_ref` field is valid `RecordRef` canonical serialization; (c) re-running `/distill --snapshot-ref <ref_from_manifest>` succeeds and produces expected staged file; (d) re-running same retry again is idempotent (dedup). (SY-65)
- `/quicksave` direct-write tool path (VR-4A-29): Instrument `/quicksave` execution. Assert: the skill invokes the Write tool (not Bash) to create the checkpoint file. Alternative method: mock the Bash tool to raise on invocation; run `/quicksave`; assert checkpoint created without triggering Bash mock. This ensures the [direct-write path](enforcement.md#direct-write-path-authorization) is taken, not a Bash engine route. (SY-67)
- Text search result ordering (VR-4A-30): Create fixtures with 3 entries in each subsystem at different `created_at` timestamps. Run `query(text="<match>")`. Assert: within each subsystem group, results are ordered by `created_at` descending. (SY-68)
- `.diag` unknown schema_version handling (VR-4A-31): Construct a `.diag` file with two entries — one with `schema_version: "1.0"`, one with `schema_version: "99.0"`. Run `/triage`. Assert: entry with `"99.0"` treated as opaque (logged but not interpreted); entry with `"1.0"` interpreted normally. (SY-60) Add edge case: `.diag` file where ALL entries have unrecognized `schema_version` (all treated as opaque). Assert `/triage` surfaces `'ledger unavailable in session <session_id>'` — the conservative approach. A non-empty `.diag` file implies hook failures occurred.
- Twice-per-reader scan count (VR-4A-32): Instrument `NativeReader.scan()` with a call counter. Run `query()` against a fixture with files in both shared and private roots. Assert: each reader's `scan()` is called exactly twice — once with `root_type="shared"`, once with `root_type="private"`. (SY-61) Degradation case: set private root to non-existent path. Assert: `scan("private")` is still called exactly once per reader (with reader returning `[]`). Assert: `diagnostics.degraded_roots == ["private"]`.
- WorktreeID resolution failure diagnostic (VR-4A-36): (a) Mock `identity.get_worktree_id()` to raise. Assert: `engram_session` does not block session startup and output goes to stderr (not to `.diag`). (b) Mock `git rev-parse --git-dir` to succeed but return empty string. Assert: `worktree_id` diagnostic is written to `.diag`. (SY-72)
...
- save_recovery.json success-path write (VR-4A-34): Run `/save` where all sub-operations succeed. Assert: `save_recovery.json` exists with `results.snapshot.status == "ok"`, `results.defer.status == "ok"`, `results.distill.status == "ok"`. Assert: `"schema_version" not in recovery_data` (explicit absence check, not just non-assertion). (SY-76)
...
- engram_quality Edit-path file-missing warning (VR-4A-37): Delete a snapshot file immediately after Edit. Assert: exit code 0, warning contains `"snapshot file not found at post-write readback"`. See [enforcement.md §Quality Validation](enforcement.md#quality-validation).
- **VR-NEW-3 (Guard degraded-mode blocking):** Mock `git rev-parse --git-dir` to fail. Invoke `engram_guard` with a Bash engine call (branch 1 path). Assert exit code 2 with diagnostic containing `"worktree_id unavailable"`. Separately, invoke with Write to a non-protected path (branch 4). Assert allowed. Verifies branches 1-2 block and branches 3-4 are unaffected. Include a branch 2 test case: Write to a Context snapshot path when `context_direct_write_authorization` is active and `worktree_id` resolution fails (degraded mode). Assert the write is blocked by branch 2 (exit code 2). Also test: same write when `context_direct_write_authorization` is inactive (before Step 4a) — assert branch 2 is a no-op, write allowed via branch 4. Add test case: mock `session_id` as unavailable from the Claude Code session context. Assert branches 1 and 2 block (exit code 2) with diagnostic containing `'session_id unavailable'`. Assert branches 3 and 4 evaluate normally (no `session_id` dependency).
...
- VR-5-5: All skills in the progressive activation manifest pass their smoke test (final progressive gate). The Step 5 active skill set equals the Step 4a set (13 skills — no new skills activate at Step 5). VR-5-5 verifies the Step 4a skill set remains functional after cleanup operations.

- **VR-NEW-10 (co-deployment promote-time validation):** The promote script must verify that both `hooks/` and `scripts/` directories are present in the deployed plugin root. If either is missing, abort with diagnostic. Unit test: invoke promote with `scripts/` directory absent from the plugin root. Assert promote fails with a specific error message referencing the missing directory. See [co-deployment invariant](enforcement.md#step-1-injection-pretooluse). (Note: VR-2A-NEW-9 provides earlier coverage at Step 2a. This Step 5 instance is a final gate check.)

VR-5-1 through VR-5-4 are grep/filesystem assertions. They may be implemented as pytest tests or as shell assertions in a CI script. The implementer chooses the framework; the pass criteria are the same.
...
**HARNESS_EXCEPTIONS mechanism:** Maintain a `HARNESS_EXCEPTIONS` list in `tests/compatibility/exceptions.py`. Each entry is `{fixture_name: str, reason: str}` documenting an intentional behavioral difference between old and new engines. The harness asserts `len(HARNESS_EXCEPTIONS) <= 5` at test collection time, matching the cap in VR-3A-6.
...
**Triage minimum:** The triage must identify at least 50 compatibility-critical fixtures. If fewer than 50 are identified, the triage methodology must be reviewed before proceeding to Step 3a. The ~100-150 target is the expected outcome; 50 is the floor.
...
| Version space coverage | All 6 version spaces (envelope, record provenance, ledger format, knowledge entry, promotion state, work audit trail) have at least one VR-* rejection/degradation test before Step 5 is complete | Progressive |

**Version space coverage map:** envelope → VR-0A-3; record provenance → VR-NEW-9; ledger format → VR-4A-22; knowledge entry → T1-gate-2; promotion state → VR-4A-21; work audit trail → VR-NEW-8.
...
**Minimal observable output per skill (SY-66):**

| Skill | Minimal Assertion |
|-------|-------------------|
| `engram init` | Run in isolated temp directory (no existing `.engram-id`). `.engram-id` created, valid UUIDv4 content, exit code 0. If isolation infeasible, assert "created or already initialized" with valid UUIDv4 in file. |
