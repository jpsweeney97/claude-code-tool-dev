...
- `engram init` idempotency matrix (VR-0B-1): (a) `.engram-id` absent → creates file with valid UUIDv4, stages for commit; (b) `.engram-id` present with valid UUID → no-op, exits 0, prints "already initialized"; (c) `.engram-id` present with malformed content → error with message instructing manual repair or re-init with `--force`; (d) `engram init --force` with malformed `.engram-id` → overwrites file with valid UUIDv4, exits 0 (SY-45); (e) `engram init --force` with valid `.engram-id` → specify and test the chosen behavior: either refuse with warning `"this will change repo_id"` or require an additional `--yes` flag to confirm. The test must assert the specified behavior (refusal or confirmation-required) and verify the `.engram-id` is only overwritten when the correct interaction is completed.
