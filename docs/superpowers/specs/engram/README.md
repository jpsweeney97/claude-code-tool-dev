...
**Precedence:** `operations` wins over `skill-contract` for `behavior_contract` claims. `data-contract` wins for all `interface_contract` and `persistence_schema` claims. See `spec.yaml` for full precedence and boundary rules.
