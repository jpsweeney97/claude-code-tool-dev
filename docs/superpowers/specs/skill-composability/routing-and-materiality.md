...
**Processing order per entry:** (1) Apply ordered correction rules 1-5 to `(affected_surface, material, suggested_arc)` tuple. (2) Apply `classifier_source` validation — correct invalid values to `rule` with structured warning (always recoverable). (3) Apply `materiality_source` validation — same semantics. (4) Apply `hold_reason` validation — correct invalid values to `null` with structured warning (always recoverable). `hold_reason` validation runs after the correction pipeline and `classifier_source`/`materiality_source` validation — it operates on `unresolved[]` entries only (not `feedback_candidates[]`). Steps 2-4 are recoverable validations that never trigger partial correction failure. Partial correction failure is triggered only by unexpected states in step 1 that cause rule 5 to fire. Steps 2, 3, and 4 MUST be evaluated in listed order (2, then 3, then 4). While all three are independently recoverable (no step depends on the outcome of another), consistent ordering is required for reproducible diagnostic output — a structured warning from step 2 must appear before step 3's warning in the log.
