...
> **Design decision (within Decision 7's envelope; the ticket delegates the *how*).** Codex stages content to `CONTENT_FILE="$(mktemp)"` (save `SKILL.md:58`) — a system temp path with no `.claude/handoffs` marker, so `is_handoff_path()` returns False and the advisory hook never fires on the Claude flow. Fix: skills stage to a **deterministic, recognizable** path under the primary state dir, and `is_handoff_path` recognizes it. Staging dir: `<project_root>/.claude/handoffs/.session-state/staging/` (derive from `PROJECT_ROOT`, NOT `dirname OPERATION_STATE_PATH` — verified: `OPERATION_STATE_PATH` nests 3 segments deeper at `.session-state/active-writes/<project>/<run>.json`, so a sibling-of derivation does NOT yield the 4-segment tail the recognizer needs). It is inside the already-gitignored `.claude/handoffs/` tree. **It is NOT covered by any TTL prune** — `prune_old_state_files` only sweeps top-level `handoff-*` + `transactions/*.json` and never recurses into `staging/` (execution-surfaced via code-quality review). The staged file is therefore **explicitly removed by the skill after a successful `write-active-handoff` commit** (it exists only so the advisory `PostToolUse:Write` hook can validate content pre-promotion; after the durable copy lands it has zero purpose). This restores the transient semantics the original `mktemp` had implicitly.
...
```
(Derive `STAGING_DIR` from `PROJECT_ROOT` — `$PROJECT_ROOT/.claude/handoffs/.session-state/staging` — NOT from `dirname OPERATION_STATE_PATH` (that path nests deeper than `.session-state/`). The active writer still commits from `CONTENT_FILE` to `ALLOCATED_ACTIVE_PATH` unchanged.) **After a successful `write-active-handoff`, the skill removes the staged `$CONTENT_FILE`** using the same `trash`-with-graceful-warn idiom each skill already uses for state-file cleanup (only on success; on failure the skill STOPs and leaves it for diagnosis). The skill note must state the truth: the staged file exists solely so the advisory hook can validate content before promotion and is removed post-commit — it is NOT TTL-pruned (nothing prunes `staging/`). The active writer is the only path to the durable file.
