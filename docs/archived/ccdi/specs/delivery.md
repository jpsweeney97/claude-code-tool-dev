...
|-----------|-----------|--------|
| Effective prepare yield | < 40% | `shadow_adjusted_yield` (subject to freshness guardrail; see [Diagnostics](#diagnostics)). In active mode: equivalent to `packets_surviving_precedence / packets_prepared`. The 40% threshold applies to whichever formula is active. |
...
**Freshness guardrail test:** A separate integration test file (`test_shadow_freshness_guardrail.py`, not the replay harness) MUST verify the guardrail fires correctly. The test invokes `validate_graduation.py` against a `graduation.json` (which MUST include the `shadow_adjusted_yield` field as defined in the [graduation protocol](#graduation-protocol)) where `shadow_adjusted_yield` was computed from stale classify results (same `classify_result_hash` across all turns for a topic with `shadow_defer_intent`). Assert: validator emits a warning that the freshness guardrail is not satisfied and does not use `shadow_adjusted_yield` as the graduation gate. The replay harness (Layer 2a) does not invoke `validate_graduation.py` — freshness guardrail verification requires the validator, which is an integration test concern.
...
4. `false_positive_rate` = `labeled_false_positives / total_labeled_topics`. The 10% kill threshold requires statistical confidence: label at least 100 topics before evaluating. **Sample-size escalation rule:** If the preliminary false-positive rate from the first 50 labels is ≥ 7%, double the minimum sample to ≥ 200 before finalizing the graduation decision. Do not finalize `graduation.json` with `status: "approved"` if the preliminary rate triggered this rule and `labeled_topics < 200`.

### Graduation Protocol

Shadow-to-active graduation is a manual gate with a concrete approval artifact:

1. **Preflight:** Verify that `uv run pytest tests/test_ccdi_agent_sequence.py` passes all 3 baseline behavioral tests (classify ordering, skip-when-no-candidates, --mark-injected-after-codex-reply). Layer 2b coverage is a prerequisite for Phase B graduation — do not evaluate kill criteria until this gate passes. Also verify that the unit test suite passes the `shadow_adjusted_yield` test rows (added by T1) and the following named tests: classifier tests `test_multi_alias_accumulation_phrase_suppresses_token` and `test_multi_alias_accumulation_exact_and_phrase_no_suppression` (in `test_topic_inventory.py`), and registry tests `test_overview_injected_propagation_facet_overview`, `test_overview_injected_propagation_family_context_available`, and `test_overview_injected_propagation_non_overview_facet`.
...
```json
{"status": "approved" | "rejected", "evaluated_dialogues": 12, "labeled_topics": 150, "effective_prepare_yield": 0.65, "shadow_adjusted_yield": 0.72, "avg_latency_ms": 320, "false_positive_rate": 0.04, "approver": "<name>", "timestamp": "<ISO>", "notes": "..."}
```
...
**Aggregation method:** `avg_latency_ms` is the unweighted mean of ALL `per_turn_latency_ms` entries across ALL diagnostics files (mean-of-all-turns, not mean-of-dialogue-means). `effective_prepare_yield` is computed as a global ratio: `sum(packets_surviving_precedence across all dialogues) / sum(packets_prepared across all dialogues)`. This is not a per-dialogue mean. For heterogeneous dialogues where per-dialogue mean latencies or per-dialogue yields differ, these formulas produce different values. The validator MUST use mean-of-all-turns for latency and global ratio for yield. `shadow_adjusted_yield` consistency validation against `repeat_detections_from_missing_deferral` is deferred until `repeat_detections_from_missing_deferral` is emitted in per-dialogue diagnostics. Currently, the validator checks `shadow_adjusted_yield` only against the kill-criteria threshold (≥ 40%) when the field is present. The formula correction and consistency check will be bundled as a single change — see [Shadow Mode Denominator Normalization](#shadow-mode-denominator-normalization) for the freshness guardrail.
...
**Freshness guardrail exit code:** When the freshness guardrail fires (classify_result_hash cannot be freshness-sensitive), the validator exits with code 0 and emits a warning to stderr. The warning fires only for `status: "approved"` reports where `effective_prepare_yield` has been validated against diagnostics — rejected reports and reports with unvalidated fallback metrics do not trigger the warning. This is consistent with the "fall back to raw yield" language — the guardrail degrades the metric, it does not block graduation.
...
| Approved status with `shadow_adjusted_yield` below threshold | `graduation.json` with `status: "approved"`, `shadow_adjusted_yield: 0.35` (below 40% threshold), `effective_prepare_yield: 0.65` | Exit 1, error reports "`shadow_adjusted_yield` 0.35 is below minimum threshold 0.40 for approved status" |
| Approved status with valid `shadow_adjusted_yield` and valid `effective_prepare_yield` | `graduation.json` with `status: "approved"`, `shadow_adjusted_yield: 0.72`, `effective_prepare_yield: 0.65`, all other fields consistent with diagnostics | Exit 0, validation passes |
| Approved status with preliminary rate ≥ 7% but insufficient sample | `graduation.json` with `status: "approved"`, `false_positive_rate: 0.08` (preliminary rate from first 50 labels was ≥ 7%), `labeled_topics: 120` (below escalated minimum of 200) | Exit 1 with "insufficient-power warning: preliminary false-positive rate ≥ 7% requires labeled_topics ≥ 200; got 120" |
| Approved status with `shadow_adjusted_yield` absent (freshness guardrail) | `graduation.json` with `status: "approved"`, `shadow_adjusted_yield` field absent (freshness guardrail fired), `effective_prepare_yield: 0.65` above threshold, all other fields consistent | Exit 0, validation passes — `effective_prepare_yield` used as fallback gate. Warning emitted to stderr: "shadow_adjusted_yield absent; treating omission as freshness guardrail activation" |
...
| Multi-alias accumulation: phrase and token both match same topic | Input matches both a phrase alias (weight 0.8) and a token alias (weight 0.5) for the same topic → phrase match is evaluated; token match is suppressed (same-topic, cross-type suppression) → final score is 0.8 only (phrase weight). Verifies that when phrase and token aliases both match the same topic, phrase suppresses token contribution regardless of token weight. |
| Multi-alias accumulation: exact and phrase on different aliases, no suppression | Input matches both an exact alias (weight 0.6) and a phrase alias (weight 0.7) on two distinct aliases of the same topic, where the phrase does not overlap the exact match substring → final score is sum of both weights (0.6 + 0.7 = 1.3, clamped per spec). Verifies that same-topic suppression applies only within the same alias type hierarchy, not across non-overlapping matches. |
...
| Cooldown deferral preserves consecutive_medium_count | Medium leaf topic at count=1 (first topic). Second topic on same turn consumes the cooldown slot → second topic transitions to `deferred: cooldown`, `consecutive_medium_count` remains 1 (not reset). Re-entry on next turn at medium → count=2 → injection fires. Verifies counter is not reset when a topic is deferred rather than scheduled. |
...
| Uniqueness enforcement on corrupt input | Load registry file with pre-existing duplicate entries in `coverage.injected_chunk_ids` → call `--mark-injected` → assert no additional duplicates introduced and final array length equals distinct set size. Similarly for `facets_injected`. Verifies MUST-enforce-uniqueness-on-append under valid-JSON-with-corrupt-values (not handled by the malformed-JSON → reinitialize path). |
...
| last_query_fingerprint normalization | Querying `"PreToolUse hook"` and `"pretooluse  hook"` (different case/whitespace) produce the same fingerprint. Same query with `docs_epoch="A"` produces a different fingerprint than with `docs_epoch=null`. Additionally: compute fingerprint with `docs_epoch=null`, compute fingerprint with `docs_epoch='null'` (literal string). Assert these are EQUAL (both produce the same fingerprint because null is encoded as the literal string `'null'`). This verifies the specific encoding without exposing implementation details. |
...
| `overview_injected` propagation: inject at facet=overview sets flag | Inject family topic (e.g., `hooks`) at `facet=overview` via `--mark-injected` → assert `coverage.overview_injected: true` for that topic entry |
| `overview_injected` propagation: subsequent absent→detected leaf sees `family_context_available` | After family topic injected at `facet=overview` (so `overview_injected: true` on family), a leaf topic (e.g., `hooks.pre_tool_use`) transitions to `detected` → assert `coverage.family_context_available: true` on the leaf entry |
| `overview_injected` propagation: inject at non-overview facet does not set flag | Inject family topic at `facet=schema` via `--mark-injected` → assert `coverage.overview_injected` remains `false` (or absent); subsequent leaf detection → assert `coverage.family_context_available: false` on the leaf |
...
| Active mode diagnostics omit shadow-only fields | Active mode JSON does NOT contain `packets_target_relevant`, `packets_surviving_precedence`, `false_positive_topic_detections` |
| Shadow mode diagnostics include shadow-only fields | Shadow mode JSON contains all shadow-only fields |
| `status: "unavailable"` schema | When `claude-code-docs` unavailable: only `status` and `phase` populated, all count/array fields absent |
...
| `classify` rejects `--inventory-snapshot` flag | `classify --text-file <path> --inventory-snapshot <path>` → non-zero exit with descriptive error. Per [integration.md#target-match-predicate](integration.md#target-match-predicate) flag name enforcement. |
...
| `dialogue-turn` rejects `--inventory` flag | `dialogue-turn --registry-file <path> --inventory <path>` (wrong flag name — should be `--inventory-snapshot`) → non-zero exit with descriptive error. Symmetric with the `classify rejects --inventory-snapshot flag` CLI integration test above. |
| `build-packet` rejects `--inventory` flag | `build-packet --registry-file <path> --inventory <path>` (wrong flag name — should be `--inventory-snapshot`) → non-zero exit with descriptive error. Symmetric with the `classify rejects --inventory-snapshot flag` CLI integration test above. |
...
| Agent gate matches built-in defaults | Run agent-side pre-dispatch gate AND CLI `classify` with built-in `ccdi_config.json` defaults (no overrides). Present exactly 1 high-confidence topic → both produce identical "threshold met" dispatch decision. Repeat with 2 medium-confidence same-family scenario. Verifies the hardcoded agent gate values match the built-in CLI defaults — catches drift if built-in defaults change without updating the agent. |
| Source divergence canary | Meta-test self-verification: (1) synthesize a temp module containing `if args.source == 'codex': <branch>` → `ast.parse`-based scanner returns True (source branch detected); (2) synthesize a temp module without any source branch → scanner returns False (no false positive). Verifies the `ast.parse` detection is not vacuously True for all inputs. |
...
| RegistrySeed ↔ TopicRegistryEntry durable fields | Every durable field in TopicRegistryEntry (all except attempt-local states `looked_up`, `built`) is present in RegistrySeed.entries field enumeration — schema-comparison test. **Authority note:** Registry.md is the authoritative source for the durable field enumeration. This test verifies that the implementation serializes all durable fields from registry.md into RegistrySeed.entries. data-model.md does not need to enumerate these fields — it owns only the envelope schema. A failure here indicates a missing field in the serializer, not necessarily a missing field in data-model.md. |
...
| RegistrySeed `inventory_snapshot_version` null at seed load | RegistrySeed with `inventory_snapshot_version: null` → treated as version mismatch, warning logged, best-effort field mapping applied. Same behavior for `inventory_snapshot_version: ""` (empty string) and absent field. These degenerate cases indicate a build defect per the write-time contract. |
...
| graduation.json status enum consistency | delivery.md `graduation.json` schema `status` field uses enum values that exist in integration.md shadow-mode gate algorithm | Parse the graduation validator's `VALID_STATUS_VALUES` constant and the gate implementation's conditional branches. Assert both handle identical status values. This is a code-level assertion, not a prose cross-reference. |
...
| `shadow_suppressed` field presence | For every `ccdi_trace` entry where `action != "shadow_defer_intent"`, assert `shadow_suppressed` key is present (boolean). Per [integration.md#ccditrace-output-contract](integration.md#ccditrace-output-contract). `shadow_suppressed` is the 9th required key in the per-turn trace entry schema. Implementations MUST include `shadow_suppressed` in all per-turn trace entries. |
...
| Suppressed re-detection no-op (CLI file-level) | Registry file with suppressed topic at turn N. Run `dialogue-turn` with the topic in classifier output but no re-entry trigger → assert registry FILE is unchanged using `assert_registry_unchanged` (deep JSON equality), matching the pattern used by `weak_results_scan_noop_absent.replay.json`. Covers the file I/O contract, not just the in-memory state machine. Companion to the unit-level "Suppressed re-detection no-op" test. |
...
| Graduation gate: Phase A unconditional | `graduation.json` with `status: "rejected"` → initial CCDI commit phase still fires: at `codex-dialogue` start, the registry file (from `ccdi_seed`) already has `injected` entries (post-initial-commit). Assert: the registry file passed via `ccdi_seed` contains at least one entry in `injected` state (confirming `/dialogue` performed the initial commit before delegation). This tests the post-condition observable from `codex-dialogue`'s perspective, not the `/dialogue` skill's tool-call sequence. Phase A is always active regardless of graduation status. |
...
| Cooldown config divergence | Agent uses hardcoded cooldown default (1 turn). Overlay sets `config_overrides.injection.cooldown_max_new_topics_per_turn: 3`. Assert: agent behavior uses hardcoded value (1), NOT overlay value (3). CLI respects overlay value for its own scheduling. Verifies pipeline isolation — agent config is independent of CLI config. |
...
| CLI backstop: `build-packet --shadow-mode --mark-deferred scout_priority` zero registry mutations | **Companion Layer 2b test:** Verify the `codex-dialogue` agent passes `--shadow-mode` to `build-packet --mark-deferred` when graduation status is not approved. This complements the CLI integration test above. |
...
| Shadow mode: `false_positive_topic_detections` key present and zero | Shadow mode diagnostics JSON → assert `"false_positive_topic_detections" in diagnostics["ccdi"]` (key presence) AND `assert diagnostics['ccdi']['false_positive_topic_detections'] == 0` (value check — the automated emitter always outputs 0 by construction; the value assertion guards against accidental non-zero emission). Human-labeled false-positive counts are recorded separately in the annotation file, not in this field. Unit test: `test_false_positive_field_always_zero` in the diagnostics emitter unit tests, verifying the emitter always writes 0 regardless of topic detection count. |
...
| Shadow mode: `shadow_defer_intent` trace entries emitted | Delegation envelope with `ccdi_seed`, no `graduation.json`, fixture has a candidate that would be deferred for `target_mismatch` or `cooldown` → after dialogue, read `ccdi_trace` from diagnostics and assert at least one entry with `action: "shadow_defer_intent"` is present, with correct `topic_key`, `reason`, and `classify_result_hash` fields. Verifies the diagnostic entry is produced in shadow mode per [integration.md#ccditrace-output-contract](integration.md#ccditrace-output-contract). Additionally: for any topic T that produces `shadow_defer_intent` entries on two consecutive turns with identical classifier input, assert the `classify_result_hash` values are identical across those turns. This exercises hash stability at the E2E trace level (complements the unit-level stability test in Registry Tests). **Named fixture requirement:** The two-consecutive-turn hash stability assertion (same topic, identical classifier input → identical `classify_result_hash` across turns) MUST be extracted into a named **replay harness fixture**: `shadow_defer_intent_hash_stability.replay.json` (Layer 2a, in `tests/fixtures/ccdi/`). The hash stability assertion exercises CLI-deterministic behavior (same classify input → same hash) which is testable without agent infrastructure. The assertion: two consecutive turns with identical classifier input for a topic with `shadow_defer_intent` → assert `classify_result_hash` values are identical across those turns. |

**Required fixture scenarios:**

| Fixture | Scenario |
|---------|----------|
| `happy_path.replay.json` | Single topic detected → searched → injected → committed. `trace_assertions` MUST include: `{"turn": <prepare_turn>, "action": "prepare"}` and `{"turn": <commit_turn>, "action": "inject"}`. `trace_assertions` MUST also include `assert_key_present` checks for ALL 9 required keys (`turn`, `classifier_result`, `semantic_hints`, `candidates`, `action`, `packet_staged`, `scout_conflict`, `commit`, `shadow_suppressed`) on at least one turn entry. This exercises the key-presence invariant at the replay harness level. |
...
| `shadow_skip_scout.replay.json` | Shadow-mode scout-priority abstention. Fixture with active scout target + CCDI candidate in shadow mode. Assert: `skip_scout` action with `shadow_suppressed: true`; zero `--mark-deferred` CLI invocations; no `shadow_defer_intent` entry for scout-priority. Additionally, MUST assert: no `shadow_defer_intent` entry in `ccdi_trace` has `topic_key` matching the scout-deferred topic. (Scout-priority deferrals do not emit `shadow_defer_intent` per integration.md scope note — they use `skip_scout` with `shadow_suppressed: true` instead.) |
...
| `hint_no_effect_already_injected.replay.json` | Prescriptive hint on already-injected topic → no additional injection, no state change. `final_registry_file_assertions` MUST include `{"path": "<topic>.last_seen_turn", "equals": <current_turn>}` — assertion (d) per registry.md: confirms hint was processed, not silently dropped. |
...
| `cooldown_defers_second_candidate.replay.json` | Turn with two high-confidence topics (A and B) → topic A scheduled and injected, topic B transitions to `deferred: cooldown`; subsequent turn sees topic B re-evaluated. `final_registry_file_assertions` includes `{"path": "<topicA>.consecutive_medium_count", "equals": 0}` verifying reset after injection, AND `{"path": "<topicB>.consecutive_medium_count", "equals": <count_at_deferral>}` verifying the deferred topic's counter is preserved (not reset) at the point of cooldown deferral. `trace_assertions` MUST include: `{"turn": <cooldown_turn>, "action": "skip_cooldown"}` for the deferred candidate. |
...
| `intra_turn_hint_ordering.replay.json` | The fixture uses a **two-turn design** to avoid mid-turn assertions: Turn 1 — `dialogue-turn` with two hints (contradicts_prior adding F, then extends_topic at F). The harness does NOT call `build-packet` on turn 1 (no `search_results` provided for the `pending_facet` candidate). Turn 2 — no hints, no classifier output. `final_registry_file_assertions` after turn 2: `{"path": "T.pending_facets", "equals": [F]}` (still pending, never served because no build-packet committed it). This proves hint 1 was processed before hint 2 without requiring mid-turn snapshots. |
| `intra_turn_hint_ordering_reversed.replay.json` | Same fixture as `intra_turn_hint_ordering.replay.json` with hint order reversed: (1) `extends_topic` on topic T at facet F (which is NOT yet in `pending_facets`); (2) `contradicts_prior` on topic T adding facet F to `pending_facets`. Assert: `dialogue-turn` produces a `facet_expansion` candidate (not `pending_facet`) — because when hint 1 (`extends_topic`) is processed, F is not yet in `pending_facets`, so the cascade resolves differently than in the forward-order fixture. The different candidate result between this fixture and `intra_turn_hint_ordering.replay.json` proves that intra-turn hint ordering is observable and that hints are processed sequentially in the order they appear in the hints file. |
| `multi_entry_epoch_scan_discrimination.replay.json` | Registry with 3 `suppressed:weak_results` entries: A at `suppressed_docs_epoch="old"`, B at `suppressed_docs_epoch="old"`, C at `suppressed_docs_epoch="different"`. Inventory snapshot `docs_epoch="new"` (changed from "old"). All 3 topics absent from classifier output. Assert: A and B transition to `detected` (epoch changed from their stored value); C remains `suppressed` (epoch mismatch is relative to each entry's `suppressed_docs_epoch`, not registry-wide). `final_registry_file_assertions`: `{"path": "A.state", "equals": "detected"}`, `{"path": "B.state", "equals": "detected"}`, `{"path": "C.state", "equals": "suppressed"}`. |
...
**Feasibility pre-test requirement:** Before declaring a mechanism viable, verify that exit code 2 from a `PreToolUse` hook actually blocks the tool call in the test environment. Register a synthetic `PreToolUse` hook that unconditionally returns exit code 2 with a block decision, invoke the target tool (e.g., a `Bash` command with a distinct marker), and assert the tool call did not execute. This pre-test guards against test environments where hook exit code 2 is silently ignored (fail-open), which would produce false-positive behavioral test results.

**Contingency clause:** If neither the primary mechanism nor the fallback produces a passing `test_layer2b_mechanism_selection` (i.e., both exit code 2 interception and PATH injection fail in the test environment), raise a blocking issue before Phase B begins. Layer 2b tests cannot be substituted with Layer 2a tests — agent sequencing is not testable via the replay harness. Do not proceed to Phase B or evaluate graduation kill criteria until this blocking issue is resolved.
