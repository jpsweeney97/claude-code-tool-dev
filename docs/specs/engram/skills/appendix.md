...
5. ~~**`query` output field richness**~~ — **CLOSED (Codex dialogue #24).** `SearchHit` is intentionally thin — it serves as a cross-entity discovery type, not a replacement for domain-specific result types. Enrichment via native tools (`task_query`, `lesson_query`) using `task_ids[]`/`lesson_ids[]` batch parameters is the designed path. This is not an N+1 problem: batch ID lookup is a single call per entity type. The enrichment rule and non-goal are codified in [foundations.md](../foundations.md#read-tool-selection-policy). No `include_summary` flag or polymorphic return type in v1.
