...
Treat the decisions below as the current authority; the Provenance section
...
3. **Identity model = A2-split.** Two *separate* identifiers, decided
   ...
   - **`session_id` frontmatter field — restore `${CLAUDE_SESSION_ID}`.** In
     ...
     it (`references/handoff-contract.md`: "Required: from `${CLAUDE_SESSION_ID}`";
     7 sites). Restoring it introduces **zero** session-identity seam into the
...
5. **Storage topology: primary = `.claude/handoffs/`; legacy =
   ...
   one-token change in scope: every storage-authority consumer,
   `test_storage_layout.py`, release-metadata tests, `storage_authority*`, and
   `references/ARCHITECTURE.md` retarget atomically (Execution step 2). Because
...
2. **Storage topology (highest-risk, do FIRST):** `storage_layout.py`
   `.codex`→`.claude`; retarget every storage-authority consumer +
   `test_storage_layout.py` + release-metadata tests + `storage_authority*` +
   `references/ARCHITECTURE.md`, atomically. Legacy stays `docs/handoffs/`
   (vestigial straggler classifier per Decision 5).
...
5. **Hooks (Option A):** re-wire `SessionStart→cleanup` +
   ...
   it — Claude never Writes the final path).
...
(AC ↔ Execution-step map: AC1→step2/AC-wide, AC2→steps 2-4, AC3→steps 2/6,
...
2. No `turbo_mode_handoff_runtime`, `.codex/handoffs/`, `.codex-plugin`, or
   `Future-Codex` strings remain (`rg` sweep clean) **and** a repo-wide
   `rg 'docs/handoffs'` external-consumer survey is completed: every hit outside
   the plugin (sibling-plugin ARCHITECTURE docs, memory pointers, scripts,
   other skills) is either remediated to `.claude/handoffs/` or explicitly
   recorded as intentionally legacy-pointing.
