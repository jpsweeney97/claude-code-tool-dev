...
- Partially: current-facing docs are more consistent after the dialogue fork resolution (D-01 addressed), unknown-request terminalization decision (D-02 addressed — see `decisions.md §Unknown Request Kinds`), advisory widening future-scope annotation (D-03 addressed), and audit-event contract alignment (D-07 addressed — contract follows code, analytics script/test fixed). No remaining spec contradictions in the original P1 finding set.
...
Fix type: resolved via contract-follows-code alignment — spec narrowed to actual model, undocumented actions added, analytics script/test fixed for deny counting correctness.
...
- Stale current-facing docs: ~~spec fork sections (D-01 addressed)~~, ~~spec unknown-request sections (D-02 addressed)~~, ~~spec advisory widening sections (D-03 addressed — restructured into current/future-scope)~~, ~~audit contract / docs (D-07 addressed — contract-follows-code alignment)~~, package README, delivery inventory, `/delegate` skill rendering, and some still-linked historical tickets / diagnostics (see findings D-05, D-06, D-08, D-09).
