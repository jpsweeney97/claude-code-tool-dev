...
- Stale current-facing docs: ~~spec fork sections (D-01 addressed)~~, spec unknown-request / widening sections (D-02, D-03 still open), package README, delivery inventory, `/delegate` skill rendering, audit contract / docs, and some still-linked historical tickets / diagnostics (see findings D-02 through D-09).
...
1. ~~Patch fork spec contradictions.~~ D-01 addressed via Dialogue Fork Scope decision. Remaining spec contradictions: unknown-request handling (D-02), advisory widening (D-03), audit-event contract (D-07) (`contracts.md`, `decisions.md`, `advisory-runtime-policy.md`, `recovery-and-journal.md`).
