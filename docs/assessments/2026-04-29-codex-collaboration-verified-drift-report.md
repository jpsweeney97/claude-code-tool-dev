...
> **Addressed-status note (2026-04-29):** Resolved in commit `7429e470` via [decisions.md §Dialogue Fork Scope](../superpowers/specs/codex-collaboration/decisions.md#dialogue-fork-scope): dialogue remains architecturally branchable via deferred copy-and-diverge (`seed_from` on `codex.dialogue.start`); `codex.dialogue.fork` as a standalone tool is permanently replaced. Spec edits applied across `foundations.md`, `contracts.md`, `delivery.md`, `recovery-and-journal.md`, current-state doc, and reconciliation register.
