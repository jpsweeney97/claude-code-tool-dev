...
- No: the current `codex-collaboration` state is not reliably understandable from the repo as-is. The reconciliation register is the closest current index, but it omits at least one live high-priority ticket (`T-20260429-02`) and assumes a cleaner ticket split than the repo actually has (`docs/status/codex-collaboration-reconciliation-register.md:7-30`, `:53-93`; `docs/tickets/2026-04-29-codex-collaboration-unsupported-server-request-reachability.md:1-79`; `docs/tickets/2026-04-23-deferred-same-turn-approval-response.md:1-18`; `docs/tickets/2026-03-30-codex-collaboration-promotion-flow-and-delegate-ux.md:1-15`).
- No: current-facing docs are not internally consistent. The biggest contradictions are dialogue fork support, unknown-request handling, and advisory widening / rotation (`docs/superpowers/specs/codex-collaboration/foundations.md:169-176`; `docs/superpowers/specs/codex-collaboration/contracts.md:18-24`, `:352-360`; `docs/superpowers/specs/codex-collaboration/decisions.md:122-148`; `docs/superpowers/specs/codex-collaboration/recovery-and-journal.md:157-166`; `docs/superpowers/specs/codex-collaboration/advisory-runtime-policy.md:32-118`).
...
- Highest-risk drifts: `codex.dialogue.fork` still appears as a current tool in spec text but is deliberately absent from code, unknown server requests are specified as escalations in one place and terminalized in another, and advisory widening is specified as active policy but explicitly rejected in code (`docs/superpowers/specs/codex-collaboration/contracts.md:18-24`, `:352-360`; `docs/superpowers/specs/codex-collaboration/decisions.md:122-148`; `packages/plugins/codex-collaboration/tests/test_mcp_server.py:21-23`; `packages/plugins/codex-collaboration/server/delegation_controller.py:978-1070`; `packages/plugins/codex-collaboration/server/control_plane.py:154-158`).
...
- Implementation / tests / skill docs: package README, skills, references, server modules, tests. Status: current primary implementation evidence (`packages/plugins/codex-collaboration/README.md:1-93`; `packages/plugins/codex-collaboration/server/mcp_server.py:18-187`; `packages/plugins/codex-collaboration/tests/test_mcp_server.py:12-23`).
...
|---|---|---|---|---|---|---|
| D-01 | P1 | verified | current-facing contradiction | `foundations.md`, `contracts.md`, `decisions.md`, `delivery.md`, `mcp_server.py`, tests | Fork is documented as a current dialogue tool but deliberately absent from code | Fix the source spec / tool-surface docs or land fork |
| D-02 | P1 | verified | current-facing contradiction | `decisions.md`, `recovery-and-journal.md`, `contracts.md`, `delegation_controller.py`, `T-20260429-02` | Unknown requests are specified as escalations in one place and terminalized in another | Choose one behavior and align docs / code / tests |
| D-03 | P1 | verified | implementation-doc mismatch | `advisory-runtime-policy.md`, `control_plane.py`, `profiles.py`, tests | Advisory widening / rotation is specified as live behavior but rejected in implementation | Mark as future-scope or implement rotate / freeze |
| D-04 | P1 | verified | roadmap / open-work accounting gap | register, `README.md`, schema-delta doc, `T-20260429-02` | The register omits a live high-priority open ticket | Add / register T-20260429-02 and reprioritize |
...
Fix type: source doc edit plus possible code / test follow-up if the intended behavior is escalation rather than terminalization.

### D-03

Current claim: advisory policy widening, narrowing, freezing, rotation, and reap conditions are active normative runtime behavior (`docs/superpowers/specs/codex-collaboration/advisory-runtime-policy.md:32-118`). Conflict: the control plane hard-rejects `network_access=True` because widening is “not implemented in R1”, tests pin that rejection, and the profile resolver rejects any sandbox / approval widening until freeze-and-rotate exists (`packages/plugins/codex-collaboration/server/control_plane.py:151-158`; `packages/plugins/codex-collaboration/tests/test_control_plane.py:631-647`; `packages/plugins/codex-collaboration/server/profiles.py:148-158`).

Why it matters: the active policy doc reads like current behavior, but implementation is still fixed read-only / never.

Fix type: source doc edit or code follow-up.

### D-04

Current claim: the reconciliation register is the working index of still-open and still-unreconciled work (`docs/status/codex-collaboration-reconciliation-register.md:7-30`). Conflict: its active / open sections do not include `T-20260429-02` (`docs/status/codex-collaboration-reconciliation-register.md:53-93`), even though the spec README and schema-delta doc both point to that ticket as active tracking and the ticket itself is open / high-priority (`docs/superpowers/specs/codex-collaboration/README.md:56-64`; `docs/superpowers/specs/codex-collaboration/2026-04-29-codex-app-server-0.125.0-schema-delta.md:35-39`; `docs/tickets/2026-04-29-codex-collaboration-unsupported-server-request-reachability.md:1-79`).
...
Fix type: reconciliation-register update.
...
- Stale current-facing docs: spec fork / unknown / widening sections, package README, delivery inventory, `/delegate` skill rendering, audit contract / docs, and some still-linked historical tickets / diagnostics (see findings D-01 through D-09).
- Missing tracker artifacts: no explicit follow-up tickets for benchmark losses `L1/L2/L3` yet (`docs/status/codex-collaboration-reconciliation-register.md:68-72`), and no register row for open T-20260429-02 (D-04).
...
1. Patch source-of-truth spec contradictions first: fork, unknown-request handling, advisory widening, audit-event contract (`foundations.md`, `contracts.md`, `decisions.md`, `advisory-runtime-policy.md`, `recovery-and-journal.md`).
...
4. Update `docs/status/codex-collaboration-reconciliation-register.md` last: add T-20260429-02, widen the closed-ticket-path warning, and remove only rows whose source artifacts were actually fixed.
