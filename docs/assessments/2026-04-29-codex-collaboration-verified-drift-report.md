...
- Partially: current-facing docs are more consistent after the dialogue fork resolution (D-01 addressed) and unknown-request terminalization decision (D-02 addressed — see `decisions.md §Unknown Request Kinds`), but the remaining contradiction — advisory widening / rotation (D-03) — is still unresolved (`docs/superpowers/specs/codex-collaboration/advisory-runtime-policy.md:32-118`).
...
- Stale current-facing docs: ~~spec fork sections (D-01 addressed)~~, ~~spec unknown-request sections (D-02 addressed)~~, spec advisory widening sections (D-03 still open), package README, delivery inventory, `/delegate` skill rendering, audit contract / docs, and some still-linked historical tickets / diagnostics (see findings D-03 through D-09).
...
1. ~~Patch fork spec contradictions.~~ D-01 addressed via Dialogue Fork Scope decision. ~~Unknown-request handling (D-02) addressed via Unknown Request Kinds decision.~~ Remaining spec contradictions: advisory widening (D-03), audit-event contract (D-07) (`advisory-runtime-policy.md`, `recovery-and-journal.md`, `contracts.md`).
