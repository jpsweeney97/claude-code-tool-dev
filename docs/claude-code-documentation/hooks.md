...
In addition to settings files and plugins, hooks can be defined directly in [skills](skills.md) and [subagents](subagents.md) using frontmatter. These hooks are scoped to the component's lifecycle and only run when that component is active.
...
* `Task` - Subagent tasks (see [subagents documentation](subagents.md))
* `Bash` - Shell commands
