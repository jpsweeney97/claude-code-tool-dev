...
Monorepo for developing Codex extensions: skills, commands, agents, hooks, plugins, and MCP servers.

## How This Repo Works

- Develop extensions in `.Codex/` and `packages/`
- Promote to `~/.Codex/` when ready
...
MCP server providing cross-model dialogue with OpenAI Codex. Enables Codex to consult an independent model for second opinions on architecture, plans, and decisions. Value: an independent model catches blind spots and challenges assumptions that a single model working alone would miss. The codex-dialogue agent was built on this integration to manage structured multi-turn conversations with a running ledger, convergence detection, and confidence-annotated synthesis.
...
**Hook delivery:** `PostToolUseFailure` `additionalContext` is confirmed working — appears as a `PostToolUseFailure:Bash hook additional context:` system-reminder in Codex's context (verified 2026-02-17).
...
Persistent knowledge capture across Codex conversations. Insights from Codex-Codex disagreements and resolutions are captured as learning cards — template-constrained, linted artifacts — and re-injected into future consultations as weak priors. Over time, consultations improve because the shared knowledge base grows from real resolutions.
...
```bash
uv run scripts/promote <type> <name>   # Validate and deploy to ~/.Codex/
```
...
Codex plugin install cross-model@turbo-mode
```
...
Read `.Codex/rules/methodology/frameworks.md` for guidance on when to use which framework. Full frameworks with templates and worked examples live in `docs/frameworks/`.
...
- `*/.Codex/handoffs/*`, `*/.Codex/notes/*`
- Gitignored paths (no commit anyway)

Full details: `.Codex/rules/workflow/git.md`
...
- **Dev vs production**: Edit extensions in `.Codex/` (this repo), not `~/.Codex/` (production). Promote when ready.
- **Sync after hook changes**: Run `uv run scripts/sync-settings` after modifying hooks — Codex reads from `settings.json`, not hook files directly.
...
- **Rules file size**: `.Codex/rules/` files auto-load into every session. Keep them minimal — move reference material to `docs/` and link to it.

## Writing Extensions

Applies to instruction documents: skills (`.Codex/skills/*/SKILL.md`) and subagents (`.Codex/agents/*.md`).

Audience is Codex. Optimize for machine parsing.
