...
Claude skill or skill-support file, this skill also wins over generic
`scrutinize`, even if the user used natural-language scrutiny wording.

- Use this skill for adversarial review of a Claude skill, skill directory,
  `SKILL.md`, behavior-shaping reference, example, or
  proposed skill contract. This skill wins over `scrutinize` for Claude skill
...
- muddled instructions make Claude improvise important behavior
...
- `structural`: parseable frontmatter, valid YAML, expected files, references
  that exist
- `behavioral`: realistic dry runs, examples, or reasoning that shows Claude
...
2. **Behavior Read** - Summarize in plain language what Claude is supposed to do
