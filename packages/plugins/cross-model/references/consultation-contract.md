...
6. **Egress sanitization:** no outbound payload to Codex without a sanitizer pass. `sanitizer_status` must be `pass_clean` or `pass_redacted`. Safety Pipeline (§7) enforces this.
   - **Defense-in-depth layer:** Context-injection scouts are additionally bound by invariants CI-SEC-1, CI-SEC-2, CI-SEC-3, CI-SEC-4, CI-SEC-5, CI-SEC-6 (see `context-injection-contract.md` Security Invariants). These are independent guarantees — the egress sanitizer and the context-injection redaction pipeline are separate defense layers.
