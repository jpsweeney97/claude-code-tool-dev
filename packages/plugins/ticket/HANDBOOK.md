...
The ticket plugin provides structured work-tracking for Claude Code sessions. It manages tickets as markdown files with YAML frontmatter, enforcing a 4-stage mutation pipeline (classify → plan → preflight → execute) that gates both user and agent writes.
...
| `scripts/ticket_paths.py` | Project root discovery (nearest `.claude/` or `.git/`); path containment validation |
...
- A project with a `.git/` or `.claude/` directory (required for project root discovery)
- Plugin installed at `~/.claude/plugins/ticket/` or via `claude plugin install`

### Autonomy Configuration

Create `.claude/ticket.local.md` at the project root to configure agent behavior. The file must include YAML frontmatter:
...
If `.claude/ticket.local.md` is absent, the plugin defaults to `autonomy_mode: suggest` and `max_creates_per_session: 5`.

### Environment Variables

| Variable | Default | Purpose |
|----------|---------|---------|
| `CLAUDE_PLUGIN_ROOT` | (auto-detected) | Plugin root override; guard falls back to own parent directory if unset |
...
1. **Preflight reads** `autonomy_mode` from `.claude/ticket.local.md`. If mode is `suggest`, agent mutations are blocked entirely.
...
**When to use**
The primary interface for all ticket operations in a Claude Code session. Use when creating, updating, closing, reopening, listing, or querying tickets from natural language. Delegates mutations to `ticket_engine_user.py`; read operations call `ticket_read.py` directly.

**Flow**
1. Resolve plugin root from `CLAUDE_PLUGIN_ROOT` or skill's own directory
...
| `error: path_traversal` | `tickets_dir` resolves outside project root | Confirm project has `.git/` or `.claude/` at expected root |
...
| `state: error`, `autonomy_blocked` | Agent mutations attempted under `suggest` mode | Set `autonomy_mode: auto_audit` in `.claude/ticket.local.md` |
...
| `denied: path_outside_cwd` | Payload path resolves outside working directory | Launch Claude Code from the project root containing `.git/` or `.claude/` |
...
Input payload (JSON)
 ...
 │             │  .claude/ticket.local.md. Blocks agent mutations if
...
|---------|-------------|-----------|---------|
| All agent mutations rejected | `autonomy_mode: suggest` (default) | Check `.claude/ticket.local.md`; absent file → defaults to `suggest` | Set `autonomy_mode: auto_audit` in `.claude/ticket.local.md` |
...
| `session_cap_exceeded` | Agent created ≥ `max_creates_per_session` tickets this session | Check `max_creates_per_session` in `.claude/ticket.local.md` | Raise cap or start a new session |
...
| `path_outside_cwd` from hook | Project root not at Claude Code launch directory | Check hook's `event.cwd` vs actual tickets path | Launch Claude Code from the project root containing `.git/` or `.claude/` |
...
```bash
grep -r "ticket_engine_guard" ~/.claude/settings.json
