...
A Claude Code plugin for structured ticket management — creating, updating, closing, triaging, and auditing work tickets from within Claude Code sessions.
...
| `/ticket-triage` | Auto-triggerable by Claude | Read-only health dashboard and audit summary |
...
Controls agent-origin behavior. Configured per-project in `.claude/ticket.local.md` (YAML frontmatter):
...
|----------|---------|---------|
| `CLAUDE_PLUGIN_ROOT` | Parent of `hooks/` directory | Absolute path to plugin root; used by hook and skills to locate scripts |
...
| `.claude/ticket-tmp/` | Temporary payload files during pipeline execution |
...
Claude guides you through the 4-stage pipeline: classifies intent, plans the ticket (checking for duplicates), runs preflight validation, and executes after confirmation.
...
| Field | Purpose |
|-------|---------|
| `session_id` | Ties the mutation to a specific Claude session |
...
1. **Autonomy policy** in `.claude/ticket.local.md` must be `auto_audit` or higher
...
1. Create `.claude/ticket.local.md` with `autonomy_mode: auto_audit` in YAML frontmatter
