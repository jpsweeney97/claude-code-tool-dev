...
Handoff is a skill-only Claude plugin for session continuity. It helps Claude save, load, and search Markdown handoffs under the current project's `.claude/handoffs/` directory.
...
1. Create `<project_root>/.claude/handoffs/` if needed.
...
The plugin does not add gitignore rules, stage files, commit files, auto-prune files, or manage cross-machine continuity. Whether `.claude/handoffs/` is tracked or ignored remains host-repository policy.
...
These headings are prompts, not a schema. The quality bar is the resumption test: could future Claude continue without wasting time, repeating avoidable exploration, or trusting a stale claim?
