...
Create a Markdown handoff at `<project_root>/.claude/handoffs/` so a future Claude session can resume without avoidable re-exploration.
...
- project-arc context: where the broader project stands, why this session matters, what prior decisions are load-bearing, and what future Claude should not forget
...
2. Create `<project_root>/.claude/handoffs/` if needed.
...
- Whether `.claude/handoffs/` is tracked or ignored remains host-repository policy.
