...
If no `.claude/handoffs/` directory exists, report:
...
```bash
rg -n --context 3 --fixed-strings "<query>" "$PROJECT_ROOT/.claude/handoffs"
```
...
```bash
rg -n --context 3 "<pattern>" "$PROJECT_ROOT/.claude/handoffs"
```
