...
**Fixed (production‑grade: shared helpers, URL + file path, preserves hyphens)**:
```typescript
...
function urlPathSegments(urlString: string): string[] {
  try {
...
function docsContentSegments(segments: string[]): string[] {
  // Strip "/docs/{lang}" if present, otherwise return as-is.
  const docsIndex = segments.indexOf('docs');
  if (docsIndex >= 0) {
    return segments.slice(Math.min(docsIndex + 2, segments.length));
  }
  return segments;
}
...
export function deriveCategory(path: string): string {
  if (isHttpUrl(path)) {
    const segments = docsContentSegments(urlPathSegments(path));
    for (const seg of segments) {
      if (KNOWN_CATEGORIES.has(seg)) return seg;
    }
...
```
**Helper placement note**: Define `KNOWN_CATEGORIES` once (e.g., in `src/filter.ts`) and export it, then import in `frontmatter.ts`, `loader.ts`, and `chunk-helpers.ts` to avoid drift between category lists.
...
**Fixed (production‑grade): Synthesize frontmatter safely with shared helpers, omit empty fields**:
```typescript
const YAML_ESCAPE = (value: string): string => JSON.stringify(value);

function buildFrontmatter(fields: Record<string, string | undefined>): string {
  const lines = ['---'];
  for (const [key, value] of Object.entries(fields)) {
    if (value) lines.push(`${key}: ${YAML_ESCAPE(value)}`);
  }
  lines.push('---', '');
  return lines.join('\n');
}
...
function deriveIdFromUrl(url: string | undefined): string | undefined {
  ...
  const segments = docsContentSegments(urlPathSegments(url));
...
return filtered.map((s) => {
  ...
  const topic = s.title || undefined;

  const syntheticFrontmatter = buildFrontmatter({
    topic,
    id,
    category,
  });
...
export function generateChunkId(file: MarkdownFile, heading?: string): string {
  ...
  if (isHttpUrl(file.path)) {
    const segments = docsContentSegments(urlPathSegments(file.path));
    // Use normalized tail to avoid collisions (e.g., "hooks/overview" -> "hooks-overview")
    const tail = segments.join('/');
    fileSlug = slugify(tail || 'unknown');
  } else {
...
**Files**:
- `tests/golden-queries.test.ts` - Build index using `loadFromOfficial` (mocked fetch + cache) and assert top-level categories from URL paths.
- `tests/corpus-validation.test.ts` - Replace local‑corpus checks with a stable remote‑fixture validation (cached `llms-full` excerpt).
- `tests/chunk-helpers.test.ts` - Add URL-path cases that include nested pages (e.g., `/docs/en/hooks/input-schema` → `hooks-input-schema`).
- `tests/frontmatter.test.ts` - Add `deriveCategory()` cases for URLs with hyphenated categories and nested paths.
- `tests/loader.test.ts` - Assert synthetic frontmatter injects `topic`, `id`, and `category` safely (YAML-escaped).

**Example snippets** (copy‑paste ready shape):
```typescript
// frontmatter.test.ts
import { describe, it, expect } from 'vitest';
import { deriveCategory } from '../src/frontmatter.js';

describe('deriveCategory (URL paths)', () => {
  it('preserves hyphenated top-level categories', () => {
    expect(deriveCategory('https://code.claude.com/docs/en/slash-commands'))
      .toBe('slash-commands');
    expect(deriveCategory('https://code.claude.com/docs/en/plugin-marketplaces'))
      .toBe('plugin-marketplaces');
    expect(deriveCategory('https://code.claude.com/docs/en/claude-md'))
      .toBe('claude-md');
  });

  it('returns top-level category for nested pages', () => {
    expect(deriveCategory('https://code.claude.com/docs/en/hooks/input-schema'))
      .toBe('hooks');
    expect(deriveCategory('https://code.claude.com/docs/en/mcp/servers'))
      .toBe('mcp');
    expect(deriveCategory('https://code.claude.com/docs/en/agents/sub-agents'))
      .toBe('agents');
  });
});
```
...
```typescript
// chunk-helpers.test.ts
import { describe, it, expect } from 'vitest';
import { generateChunkId } from '../src/chunk-helpers.js';
import type { MarkdownFile } from '../src/types.js';

describe('generateChunkId (URL paths)', () => {
  it('uses normalized URL tail to avoid collisions', () => {
    const file: MarkdownFile = {
      path: 'https://code.claude.com/docs/en/hooks/input-schema',
      content: '',
    };
    expect(generateChunkId(file, '## PreToolUse Input'))
      .toBe('hooks-input-schema#pretooluse-input');
  });
});
```
...
describe('loadFromOfficial synthetic frontmatter', () => {
  afterEach(() => {
    vi.unstubAllGlobals();
  });

  it('injects topic/id/category with YAML-safe values', async () => {
    const mockContent = `# Hooks
...
Body content`;
    ...
    const files = await loadFromOfficial('https://example.com/docs');
    expect(files[0].content).toContain('topic: \"Hooks\"');
    expect(files[0].content).toContain('id: \"hooks-input-schema\"');
    expect(files[0].content).toContain('category: \"hooks\"');
  });
});
```
...
## Remaining Risks / Acceptance Criteria

**Must‑haves before close**:
- **Single source of truth for categories**: `KNOWN_CATEGORIES` exported from one module and imported everywhere it’s needed (filtering, category derivation, chunk IDs). No duplicated lists.
- **Deprecation alignment**: If local corpus (`docs/extension-reference`) is being deprecated, add a stable remote‑corpus fixture (cached llms‑full excerpt) and migrate tests off local files.
- **Deterministic tests**: Golden queries must use either a stable local fixture or mocked `llms-full` content to avoid flaky network‑dependent results.
- **Metadata propagation**: If synthetic frontmatter is used, confirm metadata terms (topic/id/category) flow into tokens for *all* chunks.
- **Locale robustness**: URL parsing must tolerate missing or variant `docs/{lang}` prefixes without mis‑categorizing.
- **Cache hygiene**: Document (and test) a cache invalidation strategy or explicit “refresh” path when the remote corpus format or content changes.
